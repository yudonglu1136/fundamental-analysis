...
import { nowValuationConfig } from "./config";
import { defaultNowValuationAssumptions } from "./assumptions";
import { buildNowDashboardData, resolveNowDataset } from "./calculations";
import type { NowDataset, NowFinancialPeriod, NowSourceStatus, ValuationAssumptions } from "./model";
...
type NowBacktestResult = {
  id?: string;
  status?: string;
  warnings?: string[];
  metrics?: {
    nowBuyHold?: NowBacktestMetricSet;
    spy?: NowBacktestMetricSet;
    benchmark?: NowBacktestMetricSet;
  };
  curve?: NowBacktestCurvePoint[];
};
...
type NowSubscriptionAgentResponse = {
  ticker: "NOW";
  rows: NowSubscriptionAgentRow[];
  summary: {
    rowCount?: number;
...
async function fetchJsonWithFallback<T>(paths: string[], init?: RequestInit): Promise<T> {
  const apiBase = import.meta.env.VITE_NOW_API_BASE_URL ?? import.meta.env.VITE_API_BASE_URL ?? "http://127.0.0.1:8787";
...
function sourceStatus(value: unknown): NowSourceStatus {
...
function mapNowSnapshotToDataset(snapshot: BackendSnapshotResponse, fallback: NowDataset): NowDataset | null {
  const financialRows = snapshot.financialPeriods ?? [];
  if (!financialRows.length) return null;
  const periods = financialRows.map((row): NowFinancialPeriod => {
    ...
    return {
      ...
      fiscalYear,
      fiscalQuarter,
      periodType: sourceStatus(row.periodType) === "forecast_assumption" ? "forecast" : (asString(row.periodType) as NowFinancialPeriod["periodType"]) ?? "quarter",
...
export function NowDashboard({
  ...
  const [backendDataset, setBackendDataset] = useState<NowDataset | null>(null);
  ...
  const { valuationAssumptions, handleValuationValuesChange } = useValuationAssumptionState({
    ticker: "NOW",
    defaultAssumptions: defaultNowValuationAssumptions,
    storageKey: "now-valuation-assumptions",
    onDataSourceChange,
  });
  const fallbackData = resolveNowDataset(module.data);
    ...
    async function loadSnapshot() {
      ...
      try {
        const snapshot = await fetchJsonWithFallback<BackendSnapshotResponse>([
          "/api/now/snapshot",
          "/api/stocks/now/snapshot",
        ]);
        const mapped = mapNowSnapshotToDataset(snapshot, fallbackData);
  ...
  const dashboard = useMemo(
    () => buildNowDashboardData(moduleData, scenario, valuationAssumptions as Partial<ValuationAssumptions>),
  ...
  return (
    <div className="space-y-6">
      <SectionCard
        title="ServiceNow Payments-Network Research Cockpit"
      ...
      <Tabs.Root value={tab} onValueChange={setTab}>
        ...
        <Tabs.Content value="valuation" className="mt-6 space-y-6">
          ...
          <SectionCard title="NOW Valuation Assumptions" description="Interactive controls are last by design: first inspect saved backend runs, V vs SPY history, and method outputs.">
            <InteractiveValuationDashboard
              ticker="NOW"
              config={nowValuationConfig}
              data={moduleData}
              scenario={scenario}
              currency="USD"
        ...
        <Tabs.Content value="risks" className="mt-6 space-y-6">
          ...
          <SectionCard title="Competitive Position vs ServiceNow and Alternative Rails" description="Competition is split by threat vector rather than treated as a generic fintech risk.">
...
function VHistoricalValuationPanel() {
    ...
    async function loadHistoricalValuations() {
      ...
      try {
        const payload = await fetchJsonWithFallback<VHistoricalValuationResponse>([
          "/api/now/historical-valuations?scenario=Base&modelVersion=now_v1_backend_pilot",
          "/api/stocks/now/historical-valuations?scenario=Base&modelVersion=now_v1_backend_pilot",
        ]);
  ...
  return (
    <SectionCard
      title="NOW Backend Historical Valuations"
      description="Persisted Base scenario valuation runs by ServiceNow reporting event. Each run uses the data visible as of the reporting event and the nearest prior daily price bar."
    ...
    >
      {status === "offline" ? (
        <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
          Historical data service is temporarily unavailable. Static NOW dashboard sections still render.
        </div>
      ) : null}
      <div className="grid gap-4 lg:grid-cols-4">
...
function NowSubscriptionAgentPanel() {
  const [history, setHistory] = useState<NowSubscriptionAgentResponse | null>(null);
  const [status, setStatus] = useState<"loading" | "online" | "offline">("loading");
  const [message, setMessage] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    async function loadHistory() {
      setStatus("loading");
      try {
        const payload = await fetchJsonWithFallback<NowSubscriptionAgentResponse>([
          "/api/now/subscription-agent-history?quarters=40",
          "/api/stocks/now/subscription-agent-history?quarters=40",
        ]);
        if (!active) return;
        setHistory(payload);
        setStatus("online");
  ...
  return (
    <SectionCard
      title="Backend Subscription & Agent Progress"
      description="Quarterly backend comparison of NOW subscription revenue, cRPO, Agentic AI adoption and Pro Plus attach, oldest to newest."
    >
      ...
      <div className="mb-6 grid gap-4 md:grid-cols-4">
        <ScoreBlock label="Latest Subscription" value={usdm(latest?.subscriptionRevenue)} note={latest?.label ?? "Backend"} />
        <ScoreBlock label="Latest cRPO" value={usdm(latest?.currentRpo)} note={pct(latest?.currentRpoGrowth)} />
        <ScoreBlock label="Agent ARR Proxy" value={usdm(latest?.agenticAiArr)} note={(numberFmt(latest?.agenticAiCustomers) + " customers")} />
        <ScoreBlock label="Pro Plus Adoption" value={pct(latest?.proPlusAdoptionRate)} note={("FCF margin " + pct(latest?.freeCashFlowMargin))} />
      </div>
      <div className="grid gap-6 xl:grid-cols-2">
        <ChartPanel title="Historical Subscription Revenue vs cRPO">
          <ResponsiveContainer width="100%" height={320}>
            <BarChart data={rows} margin={{ top: 10, right: 24, bottom: 12, left: 0 }}>
              ...
              <Tooltip formatter={(value: number, name: string) => [usdm(value), name]} labelFormatter={(label, payload) => {
                const row = payload?.[0]?.payload as NowSubscriptionAgentRow | undefined;
                return row ? String(label) + " / " + (row.asOfDate ?? "n/a") : String(label);
              }} />
              <Legend />
              <Bar dataKey="subscriptionRevenue" fill="#2563eb" name="Subscription revenue" />
              <Bar dataKey="currentRpo" fill="#0f766e" name="cRPO" />
            </BarChart>
          </ResponsiveContainer>
        </ChartPanel>
        <ChartPanel title="QoQ Growth: Subscription Revenue vs cRPO">
          <ResponsiveContainer width="100%" height={320}>
            <BarChart data={rows} margin={{ top: 10, right: 24, bottom: 12, left: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
              <XAxis dataKey="label" tick={{ fontSize: 11 }} interval="preserveStartEnd" />
              ...
              <Legend />
              <Bar dataKey="subscriptionRevenueQoqGrowthPct" fill="#2563eb" name="Subscription revenue QoQ" />
              <Bar dataKey="currentRpoQoqGrowthPct" fill="#0f766e" name="cRPO QoQ" />
            </BarChart>
          </ResponsiveContainer>
        </ChartPanel>
        <ChartPanel title="Subscription Mix and Pro Plus Adoption">
          <ResponsiveContainer width="100%" height={320}>
            <LineChart data={rows} margin={{ top: 10, right: 24, bottom: 12, left: 0 }}>
              ...
              <Tooltip formatter={(value: number, name: string) => [Number(value).toFixed(1) + "%", name]} />
              <Legend />
              <Line type="monotone" dataKey="subscriptionRevenueMixPct" dot={false} stroke="#2563eb" strokeWidth={2.5} name="Subscription / revenue" />
              <Line type="monotone" dataKey="proPlusAdoptionPct" dot={false} stroke="#a16207" strokeWidth={2.5} name="Pro Plus adoption" />
            </LineChart>
          </ResponsiveContainer>
        </ChartPanel>
        <ChartPanel title="YoY Growth: Subscription Revenue vs cRPO">
          <ResponsiveContainer width="100%" height={320}>
            <BarChart data={rows} margin={{ top: 10, right: 24, bottom: 12, left: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
              <XAxis dataKey="label" tick={{ fontSize: 11 }} interval="preserveStartEnd" />
              ...
              <Tooltip formatter={(value: number, name: string) => [Number(value).toFixed(1) + "%", name]} />
              <Legend />
              <Bar dataKey="subscriptionRevenueYoyGrowthPct" fill="#2563eb" name="Subscription revenue YoY" />
              <Bar dataKey="currentRpoYoyGrowthPct" fill="#0f766e" name="cRPO YoY" />
            </BarChart>
          </ResponsiveContainer>
        </ChartPanel>
      </div>
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        ...
        <InsightPanel icon={<Database className="h-5 w-5" />} title="Subscription Growth Check" text="Subscription revenue and cRPO are shown together so reported growth can be compared with forward demand, not just trailing revenue." />
      </div>
      <div className="mt-6">
        <DataTable
          headers={["Quarter", "Subscription", "cRPO", "Sub YoY", "cRPO YoY", "Agent ARR", "Pro Plus", "Source"]}
...
function VCapitalReturnsBackendPanel() {
    ...
    async function loadHistory() {
      try {
        const payload = await fetchJsonWithFallback<VCapitalReturnResponse>([
          "/api/now/capital-returns?years=8",
          "/api/stocks/now/capital-returns?years=8",
        ]);
        if (!cancelled) {
          setHistory(payload);
          setStatus("online");
  ...
  return (
    <SectionCard
      ...
      description="Eight-year NOW capital-return history from backend financial-period data. Dividends are zero for NOW; buybacks stack separately and are compared with annual FCF and are compared with annual FCF."
      badge={<span className={`rounded-full px-3 py-1 text-xs font-semibold uppercase ${status === "online" ? "bg-emerald-50 text-emerald-700" : status === "loading" ? "bg-slate-100 text-slate-600" : "bg-amber-50 text-amber-700"}`}>{status === "online" ? "Data online" : status === "loading" ? "Loading" : "Data unavailable"}</span>}
    >
      ...
      <div className="mt-5">
        <ChartPanel title="Capital Return Stack vs FCF">
          <ResponsiveContainer width="100%" height={360}>
            <BarChart data={chartRows}>
              <defs>
                <pattern id="nowDividendForecastHatch" width="8" height="8" patternUnits="userSpaceOnUse" patternTransform="rotate(45)">
              ...
              <Bar dataKey="dividendCashForecast" stackId="forecastCapitalReturn" fill="url(#nowDividendForecastHatch)" stroke="#0f766e" strokeDasharray="4 3" name="Forecast dividends" />
...
function NowBacktestPanel() {
  const [startDate, setStartDate] = useState("2018-01-02");
  const [endDate, setEndDate] = useState("2026-05-12");
  const [status, setStatus] = useState<"idle" | "running" | "done" | "error">("idle");
  const [result, setResult] = useState<NowBacktestResult | null>(null);

  const runBacktest = useCallback(async () => {
    setStatus("running");
    try {
      const payload = await fetchJsonWithFallback<NowBacktestResult>([
        "/api/now/backtests",
        "/api/stocks/now/backtests",
      ], {
        method: "POST",
  ...
  const curve = useMemo(() => {
    ...
    return rows.filter((_, index) => index % step === 0 || index === rows.length - 1).map((row) => ({
      date: row.date,
      nowReturn: (row.nowBuyHold - 1) * 100,
  ...
  return (
    <SectionCard
      title="NOW vs SPY Backtest"
      description="Select a date range and compare daily NOW buy-and-hold performance against SPY from backend price history."
          ...
          <ChartPanel title="Total Return Since Start">
            <ResponsiveContainer width="100%" height={320}>
              <LineChart data={curve}>
                ...
                <Tooltip formatter={(value: number) => `${value.toFixed(1)}%`} />
                <Legend />
                <Line type="monotone" dataKey="nowReturn" dot={false} stroke="#2563eb" strokeWidth={2.5} name="NOW" />
                <Line type="monotone" dataKey="spyReturn" dot={false} stroke="#64748b" strokeWidth={2.5} name="SPY" />
              </LineChart>
            </ResponsiveContainer>
          </ChartPanel>
          <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <ScoreBlock label="NOW CAGR" value={pct(metrics.nowBuyHold?.cagr)} note="Buy-and-hold" />
            <ScoreBlock label="SPY CAGR" value={pct(metrics.spy?.cagr)} note="Benchmark" />
            <ScoreBlock label="NOW MDD" value={pct(metrics.nowBuyHold?.maxDrawdown)} note="Maximum drawdown" />
            <ScoreBlock label="SPY MDD" value={pct(metrics.spy?.maxDrawdown)} note="Maximum drawdown" />
            <ScoreBlock label="NOW Sharpe" value={metrics.nowBuyHold?.sharpe != null ? metrics.nowBuyHold.sharpe.toFixed(2) : "n/a"} note="Zero risk-free rate" />
            <ScoreBlock label="SPY Sharpe" value={metrics.spy?.sharpe != null ? metrics.spy.sharpe.toFixed(2) : "n/a"} note="Zero risk-free rate" />
            <ScoreBlock label="NOW Vol" value={pct(metrics.nowBuyHold?.volatility)} note="Annualized daily vol" />
...
function ValuationTriangulationPanel({ valuation }: { valuation: ReturnType<typeof buildNowDashboardData>["valuation"] }) {
  const methodRows = valuation.methodCards.map((card) => ({
    method: card.label,
    value: card.value,
  }));
  return (
    <SectionCard title="NOW Valuation Triangulation / Method Outputs" description="DCF / FCFF, FCF yield, P/E, EV/Revenue, and enterprise SaaS peer-premium methods test whether V deserves a premium multiple as growth normalizes.">
      <div className="grid gap-6 xl:grid-cols-2">
