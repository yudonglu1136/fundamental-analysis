...
import { googlAssumptionDefinitions, googlScenarioDefaults } from "../assumptions";

export function GooglAssumptionsPanel({
  values,
  onChange,
  onReset,
  activeScenario,
  ...
  description = "Tune Search monetization, Cloud margin conversion, TPU efficiency, and AI capital intensity.",
}: {
  values: GooglAssumptions;
  onChange: (key: keyof GooglAssumptions, value: number) => void;
  ...
  const groups = useMemo(() => {
    const filtered = categories?.length ? googlAssumptionDefinitions.filter((item) => categories.includes(item.category)) : googlAssumptionDefinitions;
                      ...
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                          ...
                          <p className="mt-1 text-sm text-slate-500">
                            Bear {formatValue(googlScenarioDefaults.Bear[item.key as keyof GooglAssumptions], item.format, "USD")} · Base {formatValue(googlScenarioDefaults.Base[item.key as keyof GooglAssumptions], item.format, "USD")} · Bull{" "}
                            {formatValue(googlScenarioDefaults.Bull[item.key as keyof GooglAssumptions], item.format, "USD")}
                          </p>
                        </div>
                        <div className="flex items-center gap-2">
                          <p className="text-sm font-semibold text-ink">{formatValue(values[item.key as keyof GooglAssumptions], item.format, "USD")}</p>
                          <DataQualityBadge badge={item.source} />
                        </div>
                      </div>
                      <div className="mt-4 grid gap-3 lg:grid-cols-[1fr_120px]">
                        <input type="range" min={item.min} max={item.max} step={item.step} value={values[item.key as keyof GooglAssumptions]} onChange={(event) => onChange(item.key as keyof GooglAssumptions, Number(event.target.value))} className="w-full accent-cyan-600" />
                        <input type="number" min={item.min} max={item.max} step={item.step} inputMode="decimal" value={Number(values[item.key as keyof GooglAssumptions].toFixed(item.step < 1 ? 3 : 1))} onChange={(event) => onChange(item.key as keyof GooglAssumptions, Number(event.target.value))} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-ink shadow-sm focus:border-cyan-400 focus:outline-none" />
                      </div>
                    </div>
                  ))}
                </div>
