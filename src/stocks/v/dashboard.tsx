...
import { vValuationConfig } from "./config";
import { defaultVValuationAssumptions } from "./assumptions";
import { buildVDashboardData, resolveVDataset } from "./calculations";
import type { VDataset, VFinancialPeriod, VSourceStatus, ValuationAssumptions } from "./model";
...
type VBacktestResult = {
  id?: string;
  status?: string;
  warnings?: string[];
  metrics?: {
    vBuyHold?: VBacktestMetricSet;
    spy?: VBacktestMetricSet;
    benchmark?: VBacktestMetricSet;
  };
  curve?: VBacktestCurvePoint[];
};
...
type VIncentivesNetRevenueResponse = {
  ticker: "V";
  rows: VIncentivesNetRevenueRow[];
  summary: {
    rowCount?: number;
...
async function fetchJsonWithFallback<T>(paths: string[], init?: RequestInit): Promise<T> {
  const apiBase = import.meta.env.VITE_V_API_BASE_URL ?? import.meta.env.VITE_API_BASE_URL ?? "http://127.0.0.1:8787";
...
function sourceStatus(value: unknown): VSourceStatus {
...
function mapVSnapshotToDataset(snapshot: BackendSnapshotResponse, fallback: VDataset): VDataset | null {
  const financialRows = snapshot.financialPeriods ?? [];
  if (!financialRows.length) return null;
  const periods = financialRows.map((row): VFinancialPeriod => {
    ...
    return {
      ...
      fiscalYear,
      fiscalQuarter,
      periodType: sourceStatus(row.periodType) === "forecast_assumption" ? "forecast" : (asString(row.periodType) as VFinancialPeriod["periodType"]) ?? "quarter",
...
export function VDashboard({
  ...
  const [backendDataset, setBackendDataset] = useState<VDataset | null>(null);
  ...
  const { valuationAssumptions, handleValuationValuesChange } = useValuationAssumptionState({
    ticker: "V",
    defaultAssumptions: defaultVValuationAssumptions,
    storageKey: "v-valuation-assumptions",
    onDataSourceChange,
  });
  const fallbackData = resolveVDataset(module.data);
    ...
    async function loadSnapshot() {
      ...
      try {
        const snapshot = await fetchJsonWithFallback<BackendSnapshotResponse>([
          "/api/v/snapshot",
          "/api/stocks/v/snapshot",
        ]);
        const mapped = mapVSnapshotToDataset(snapshot, fallbackData);
  ...
  const dashboard = useMemo(
    () => buildVDashboardData(moduleData, scenario, valuationAssumptions as Partial<ValuationAssumptions>),
  ...
  return (
    <div className="space-y-6">
      <SectionCard
        title="Visa Payments-Network Research Cockpit"
      ...
      <Tabs.Root value={tab} onValueChange={setTab}>
        ...
        <Tabs.Content value="valuation" className="mt-6 space-y-6">
          ...
          <SectionCard title="V Valuation Assumptions" description="Interactive controls are last by design: first inspect saved backend runs, V vs SPY history, and method outputs.">
            <InteractiveValuationDashboard
              ticker="V"
              config={vValuationConfig}
              data={moduleData}
              scenario={scenario}
              currency="USD"
...
function VHistoricalValuationPanel() {
    ...
    async function loadHistoricalValuations() {
      ...
      try {
        const payload = await fetchJsonWithFallback<VHistoricalValuationResponse>([
          "/api/v/historical-valuations?scenario=Base&modelVersion=v_v1_backend_pilot",
          "/api/stocks/v/historical-valuations?scenario=Base&modelVersion=v_v1_backend_pilot",
        ]);
  ...
  return (
    <SectionCard
      title="V Backend Historical Valuations"
      description="Persisted Base scenario valuation runs by Visa reporting event. Each run uses the data visible as of the reporting event and the nearest prior daily price bar."
    ...
    >
      {status === "offline" ? (
        <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
          Historical data service is temporarily unavailable. Static V dashboard sections still render.
        </div>
      ) : null}
      <div className="grid gap-4 lg:grid-cols-4">
...
function VIncentivesNetRevenuePanel() {
  const [history, setHistory] = useState<VIncentivesNetRevenueResponse | null>(null);
  const [status, setStatus] = useState<"loading" | "online" | "offline">("loading");
  const [message, setMessage] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    async function loadHistory() {
      try {
        const payload = await fetchJsonWithFallback<VIncentivesNetRevenueResponse>([
          "/api/v/incentives-vs-net-revenue?quarters=40",
          "/api/stocks/v/incentives-vs-net-revenue?quarters=40",
        ]);
        if (!cancelled) {
          setHistory(payload);
          setStatus("online");
  ...
  return (
    <SectionCard
      ...
      description="Quarterly backend comparison of V client incentives against net revenue, oldest to newest, so take-rate quality is visible by event."
      badge={<span className={`rounded-full px-3 py-1 text-xs font-semibold uppercase ${status === "online" ? "bg-emerald-50 text-emerald-700" : status === "loading" ? "bg-slate-100 text-slate-600" : "bg-amber-50 text-amber-700"}`}>{status === "online" ? "Data online" : status === "loading" ? "Loading" : "Data unavailable"}</span>}
    >
      ...
      <div className="grid gap-4 lg:grid-cols-4">
        ...
        <ScoreBlock label="Latest Incentives" value={usdm(latest?.rebatesIncentives)} note="Client incentives" />
        ...
        <ChartPanel title="Quarterly Incentives vs Net Revenue">
          <ResponsiveContainer width="100%" height={340}>
            <BarChart data={chartRows}>
              ...
              <Tooltip
                formatter={(value: number, name: string) => {
                  const labels: Record<string, string> = {
                    ...
                    rebatesIncentives: "Client incentives",
                  };
                  return [usdm(value), labels[name] ?? name];
                }}
                labelFormatter={(label, payload) => {
                  const row = payload?.[0]?.payload as VIncentivesNetRevenueRow | undefined;
              ...
              <Bar dataKey="rebatesIncentives" fill="#0f766e" name="Client incentives" />
            </BarChart>
          </ResponsiveContainer>
        </ChartPanel>
      </div>
        ...
        <ChartPanel title="QoQ Growth: Incentives vs Net Revenue">
          <ResponsiveContainer width="100%" height={320}>
            <LineChart data={growthRows}>
              ...
              <Tooltip
                formatter={(value: number, name: string) => {
                  const labels: Record<string, string> = {
                    ...
                    rebatesIncentivesGrowth: "Client incentives QoQ",
                  };
                  return [`${value.toFixed(1)}%`, labels[name] ?? name];
                }}
                labelFormatter={(label, payload) => {
                  const row = payload?.[0]?.payload as (VIncentivesNetRevenueRow & { netRevenueGrowth?: number | null; rebatesIncentivesGrowth?: number | null }) | undefined;
              ...
              <Line type="monotone" dataKey="rebatesIncentivesGrowth" dot={false} stroke="#0f766e" strokeWidth={2.5} name="Client incentives QoQ" />
            </LineChart>
          </ResponsiveContainer>
        </ChartPanel>
      </div>
        ...
        <ChartPanel title="YoY Growth: Incentives vs Net Revenue">
          <ResponsiveContainer width="100%" height={320}>
            <BarChart data={yoyRows}>
              ...
              <Tooltip
                formatter={(value: number, name: string) => {
                  const labels: Record<string, string> = {
                    ...
                    rebatesIncentivesYoyGrowth: "Client incentives YoY",
                  };
                  return [`${value.toFixed(1)}%`, labels[name] ?? name];
                }}
                labelFormatter={(label, payload) => {
                  const row = payload?.[0]?.payload as (VIncentivesNetRevenueRow & { netRevenueYoyGrowth?: number | null; rebatesIncentivesYoyGrowth?: number | null }) | undefined;
              ...
              <Bar dataKey="rebatesIncentivesYoyGrowth" fill="#0f766e" name="Client incentives YoY" />
            </BarChart>
          </ResponsiveContainer>
        </ChartPanel>
      </div>
...
function VCapitalReturnsBackendPanel() {
    ...
    async function loadHistory() {
      try {
        const payload = await fetchJsonWithFallback<VCapitalReturnResponse>([
          "/api/v/capital-returns?years=8",
          "/api/stocks/v/capital-returns?years=8",
        ]);
        if (!cancelled) {
          setHistory(payload);
          setStatus("online");
  ...
  return (
    <SectionCard
      ...
      description="Eight-year V capital-return history from backend financial-period data. Dividends and buybacks stack into one capital-return bar and are compared with annual FCF."
      badge={<span className={`rounded-full px-3 py-1 text-xs font-semibold uppercase ${status === "online" ? "bg-emerald-50 text-emerald-700" : status === "loading" ? "bg-slate-100 text-slate-600" : "bg-amber-50 text-amber-700"}`}>{status === "online" ? "Data online" : status === "loading" ? "Loading" : "Data unavailable"}</span>}
    >
      ...
      <div className="mt-5">
        <ChartPanel title="Capital Return Stack vs FCF">
          <ResponsiveContainer width="100%" height={360}>
            <BarChart data={chartRows}>
              <defs>
                <pattern id="vDividendForecastHatch" width="8" height="8" patternUnits="userSpaceOnUse" patternTransform="rotate(45)">
              ...
              <Bar dataKey="dividendCashForecast" stackId="forecastCapitalReturn" fill="url(#vDividendForecastHatch)" stroke="#0f766e" strokeDasharray="4 3" name="Forecast dividends" />
...
function VBacktestPanel() {
  const [startDate, setStartDate] = useState("2018-01-02");
  const [endDate, setEndDate] = useState("2026-05-12");
  const [status, setStatus] = useState<"idle" | "running" | "done" | "error">("idle");
  const [result, setResult] = useState<VBacktestResult | null>(null);

  const runBacktest = useCallback(async () => {
    setStatus("running");
    try {
      const payload = await fetchJsonWithFallback<VBacktestResult>([
        "/api/v/backtests",
        "/api/stocks/v/backtests",
      ], {
        method: "POST",
  ...
  return (
    <SectionCard
      title="V vs SPY Backtest"
      description="Select a date range and compare daily V buy-and-hold performance against SPY from backend price history."
          ...
          <ChartPanel title="Total Return Since Start">
            <ResponsiveContainer width="100%" height={320}>
              <LineChart data={curve}>
                ...
                <Tooltip formatter={(value: number) => `${value.toFixed(1)}%`} />
                <Legend />
                <Line type="monotone" dataKey="vReturn" dot={false} stroke="#2563eb" strokeWidth={2.5} name="V" />
                <Line type="monotone" dataKey="spyReturn" dot={false} stroke="#64748b" strokeWidth={2.5} name="SPY" />
              </LineChart>
            </ResponsiveContainer>
          </ChartPanel>
          <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <ScoreBlock label="V CAGR" value={pct(metrics.vBuyHold?.cagr)} note="Buy-and-hold" />
            <ScoreBlock label="SPY CAGR" value={pct(metrics.spy?.cagr)} note="Benchmark" />
            <ScoreBlock label="V MDD" value={pct(metrics.vBuyHold?.maxDrawdown)} note="Maximum drawdown" />
            <ScoreBlock label="SPY MDD" value={pct(metrics.spy?.maxDrawdown)} note="Maximum drawdown" />
            <ScoreBlock label="V Sharpe" value={metrics.vBuyHold?.sharpe != null ? metrics.vBuyHold.sharpe.toFixed(2) : "n/a"} note="Zero risk-free rate" />
            <ScoreBlock label="SPY Sharpe" value={metrics.spy?.sharpe != null ? metrics.spy.sharpe.toFixed(2) : "n/a"} note="Zero risk-free rate" />
            <ScoreBlock label="V Vol" value={pct(metrics.vBuyHold?.volatility)} note="Annualized daily vol" />
...
function ValuationTriangulationPanel({ valuation }: { valuation: ReturnType<typeof buildVDashboardData>["valuation"] }) {
  const methodRows = valuation.methodCards.map((card) => ({
    method: card.label,
    value: card.value,
  }));
  return (
    <SectionCard title="V Valuation Triangulation / Method Outputs" description="DCF / FCFF, FCF yield, P/E, EV/EBIT, and payments peer-premium methods test whether V deserves a premium multiple as growth normalizes.">
      <div className="grid gap-6 xl:grid-cols-2">
