...
import { maValuationConfig } from "./config";
import { defaultMaValuationAssumptions } from "./assumptions";
import { buildMaDashboardData, resolveMaDataset } from "./calculations";
import type { MaDataset, MaFinancialPeriod, MaSourceStatus, MaValuationAssumptions } from "./model";

type MaHistoricalValuationRun = {
  id: string;
  asOfDate: string;
...
type MaHistoricalValuationEvent = {
  id: string;
  eventDate: string;
  eventType: string;
...
type MaHistoricalValuationItem = {
  event: MaHistoricalValuationEvent;
  valuationRun: MaHistoricalValuationRun | null;
};

type MaHistoricalValuationResponse = {
  historicalValuations?: MaHistoricalValuationItem[];
};

type MaBacktestMetricSet = {
...
type MaBacktestResult = {
  id?: string;
  status?: string;
  warnings?: string[];
  metrics?: {
    maBuyHold?: MaBacktestMetricSet;
    spy?: MaBacktestMetricSet;
    benchmark?: MaBacktestMetricSet;
  };
  curve?: MaBacktestCurvePoint[];
};

type MaCapitalReturnRow = {
  fiscalYear: number;
  periodId: string;
...
type MaCapitalReturnResponse = {
  ticker: "MA";
  currency: "USD";
  unit: "USDm";
  years: number;
  rows: MaCapitalReturnRow[];
  forwardExpectation?: MaCapitalReturnRow | null;
  summary: {
...
type MaIncentivesNetRevenueRow = {
  periodId: string;
  fiscalYear: number;
  ...
  incentivesToNetRevenue: number | null;
  takeRate?: number | null;
  sourceQuality: string;
};

type MaIncentivesNetRevenueResponse = {
  ticker: "MA";
  rows: MaIncentivesNetRevenueRow[];
  summary: {
    rowCount?: number;
    latestPeriod?: string | null;
    latestNetRevenue?: number | null;
    latestRebatesIncentives?: number | null;
    latestIncentivesToNetRevenue?: number | null;
    averageIncentivesToNetRevenue?: number | null;
  };
  warnings?: Array<{ id?: string; title?: string; detail?: string; severity?: string }>;
};
...
async function fetchJsonWithFallback<T>(paths: string[], init?: RequestInit): Promise<T> {
  const apiBase = import.meta.env.VITE_MA_API_BASE_URL ?? import.meta.env.VITE_API_BASE_URL ?? "http://127.0.0.1:8787";
...
function sourceStatus(value: unknown): MaSourceStatus {
...
function mapMaSnapshotToDataset(snapshot: BackendSnapshotResponse, fallback: MaDataset): MaDataset | null {
  const financialRows = snapshot.financialPeriods ?? [];
  if (!financialRows.length) return null;
  const periods = financialRows.map((row): MaFinancialPeriod => {
    ...
    return {
      ...
      fiscalYear,
      fiscalQuarter,
      periodType: sourceStatus(row.periodType) === "forecast_assumption" ? "forecast" : (asString(row.periodType) as MaFinancialPeriod["periodType"]) ?? "quarter",
  ...
  return {
    ...
    segmentFinancials: (snapshot.segmentFinancials ?? []).map((row) => ({
      ...
      segment: asString(row.segment) ?? "Unknown",
    ...
    marketData: {
      ...
      source: asString(market.source) ?? "MA backend SQLite market snapshot",
...
export function MaDashboard({
  ...
  const [backendDataset, setBackendDataset] = useState<MaDataset | null>(null);
  ...
  const { valuationAssumptions, handleValuationValuesChange } = useValuationAssumptionState({
    ticker: "MA",
    defaultAssumptions: defaultMaValuationAssumptions as unknown as Record<string, number>,
    onDataSourceChange,
  });
  const fallbackData = resolveMaDataset(module.data);
    ...
    async function loadSnapshot() {
      ...
      try {
        const snapshot = await fetchJsonWithFallback<BackendSnapshotResponse>([
          "/api/ma/snapshot",
          "/api/stocks/ma/snapshot",
        ]);
        const mapped = mapMaSnapshotToDataset(snapshot, fallbackData);
        if (!mapped) throw new Error("MA backend snapshot did not include financial periods");
        if (!cancelled) {
  ...
  const dashboard = useMemo(
    () => buildMaDashboardData(moduleData, scenario, valuationAssumptions as Partial<MaValuationAssumptions>),
  ...
  return (
    <div className="space-y-6">
      <SectionCard
        title="Mastercard Payments-Network Research Cockpit"
        description="MA is framed around cross-border recovery, switched transactions, gross dollar volume, value-added services, take-rate stability, network-fee regulation, FCF conversion, and buyback-funded EPS growth."
      ...
      <Tabs.Root value={tab} onValueChange={setTab}>
        ...
        <Tabs.Content value="dashboard" className="mt-6 space-y-6">
          <SectionCard title="Core Investment Questions" description="MA's premium multiple is tested through measurable network, mix, regulation, cash-flow, and capital-return drivers.">
            <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-4">
              ...
              <InsightPanel icon={<LineChartIcon className="h-5 w-5" />} title="Premium Multiple vs Visa and Market" text="P/E, FCF yield, EV/EBIT, and peer-premium methods show whether MA still deserves a premium as growth normalizes." />
              <InsightPanel icon={<Database className="h-5 w-5" />} title="Normalized FCF Conversion" text={latestOperatingMetric?.normalizedFcfCommentary ?? "The dashboard underwrites MA as a capex-light network but tests incentives, rebates, and regulation against FCF quality."} />
        ...
        <Tabs.Content value="volume-network" className="mt-6 space-y-6">
        ...
        <Tabs.Content value="revenue-mix" className="mt-6 space-y-6">
          <SectionCard title="Revenue Mix / Value-Added Services" description="The segment framework is analytical, not reported segment accounting: it is built to test what supports MA's premium margin and multiple.">
            <div className="grid gap-6 xl:grid-cols-2">
          ...
          <SectionCard title="Operating Leverage and Margin Durability" description="Margins and FCF are tested against transaction growth, VAS mix, rebates/incentives, FX, and regulation.">
            <div className="grid gap-6 xl:grid-cols-2">
              ...
              <div className="grid gap-4">
                ...
                <InsightPanel icon={<Database className="h-5 w-5" />} title="Normalized FCF Conversion" text={latestOperatingMetric?.normalizedFcfCommentary ?? "MA's low capex intensity supports high FCF conversion, but rebates and incentives can still matter."} />
              </div>
            </div>
          </SectionCard>
        </Tabs.Content>
        ...
        <Tabs.Content value="capital-return" className="mt-6 space-y-6">
          <MaCapitalReturnsBackendPanel />
        </Tabs.Content>

        <Tabs.Content value="valuation" className="mt-6 space-y-6">
          <MaHistoricalValuationPanel />
          <MaBacktestPanel />
          <MaValuationTriangulationPanel valuation={valuation} />
          <SectionCard title="MA Valuation Assumptions" description="Interactive controls are last by design: first inspect saved backend runs, MA vs SPY history, and method outputs.">
            <InteractiveValuationDashboard
              ticker="MA"
              config={maValuationConfig}
              data={moduleData}
              scenario={scenario}
              currency="USD"
        ...
        <Tabs.Content value="risks" className="mt-6 space-y-6">
          <SectionCard title="Risk Red Team" description="The risk module turns MA's premium multiple into disconfirmable underwriting tests.">
            <div className="grid gap-4 lg:grid-cols-2">
              <BulletPanel title="Strongest Bear Case" items={[
                ...
                "Visa, Amex, domestic networks, RTP, and account-to-account rails pressure acceptance economics and investor confidence.",
              ...
              <BulletPanel title="Falsifiers and Monitoring Triggers" items={[
                ...
                "Regulatory outcomes target merchant economics more than MA network fees.",
                "FCF comfortably covers dividends plus buybacks while diluted share count continues falling.",
              ]} />
            </div>
          </SectionCard>
          <SectionCard title="Competitive Position vs Visa and Alternative Rails" description="Competition is split by threat vector rather than treated as a generic fintech risk.">
            <div className="grid gap-4 lg:grid-cols-3">
              <InsightPanel icon={<CreditCard className="h-5 w-5" />} title="Visa / Amex" text="Visa is the closest global network comp; Amex is a closed-loop premium-spend comparator with different funding and credit economics." />
...
function MaHistoricalValuationPanel() {
  const [rows, setRows] = useState<MaHistoricalValuationItem[]>([]);
  ...
  useEffect(() => {
    ...
    async function loadHistoricalValuations() {
      ...
      setMessage(null);
      try {
        const payload = await fetchJsonWithFallback<MaHistoricalValuationResponse>([
          "/api/ma/historical-valuations?scenario=Base&modelVersion=ma_v1_backend_pilot",
          "/api/stocks/ma/historical-valuations?scenario=Base&modelVersion=ma_v1_backend_pilot",
        ]);
  ...
  return (
    <SectionCard
      title="MA Backend Historical Valuations"
      description="Persisted Base scenario valuation runs by Mastercard reporting event. Each run uses the data visible as of the reporting event and the nearest prior daily price bar."
    ...
    >
      {status === "offline" ? (
        <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
          Historical data service is temporarily unavailable. Static MA dashboard sections still render.
        </div>
      ) : null}
      <div className="grid gap-4 lg:grid-cols-4">
...
function MaIncentivesNetRevenuePanel() {
  const [history, setHistory] = useState<MaIncentivesNetRevenueResponse | null>(null);
  const [status, setStatus] = useState<"loading" | "online" | "offline">("loading");
  const [message, setMessage] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    async function loadHistory() {
      try {
        const payload = await fetchJsonWithFallback<MaIncentivesNetRevenueResponse>([
          "/api/ma/incentives-vs-net-revenue?quarters=40",
          "/api/stocks/ma/incentives-vs-net-revenue?quarters=40",
        ]);
        if (!cancelled) {
          setHistory(payload);
          setStatus("online");
  ...
  const chartRows = rows.map((row) => ({
    ...row,
    incentiveRatioPct: row.incentivesToNetRevenue == null ? null : row.incentivesToNetRevenue * 100,
  }));
  const growthRows = rows
    .map((row, index) => {
      ...
      return {
        ...row,
        netRevenueGrowth: qoqGrowthPct(row.netRevenue, previous?.netRevenue),
        rebatesIncentivesGrowth: qoqGrowthPct(row.rebatesIncentives, previous?.rebatesIncentives),
      };
    })
    .filter((row) => row.netRevenueGrowth != null || row.rebatesIncentivesGrowth != null);
  const rowByFiscalQuarter = new Map(rows.map((row) => [`${row.fiscalYear}-${row.fiscalQuarter}`, row]));
  const yoyRows = rows
    .map((row) => {
      const previousYear = rowByFiscalQuarter.get(`${row.fiscalYear - 1}-${row.fiscalQuarter}`);
      return {
        ...row,
        netRevenueYoyGrowth: qoqGrowthPct(row.netRevenue, previousYear?.netRevenue),
        rebatesIncentivesYoyGrowth: qoqGrowthPct(row.rebatesIncentives, previousYear?.rebatesIncentives),
      };
    })
    .filter((row) => row.netRevenueYoyGrowth != null || row.rebatesIncentivesYoyGrowth != null);
  const latest = rows[rows.length - 1];
  const warningText = history?.warnings?.map((warning) => `${warning.title}: ${warning.detail}`).join(" ") ?? null;

  return (
    <SectionCard
      title="Client Incentives vs Net Revenue"
      description="Quarterly backend comparison of MA rebates/incentives against net revenue, oldest to newest, so take-rate quality is visible by event."
      badge={<span className={`rounded-full px-3 py-1 text-xs font-semibold uppercase ${status === "online" ? "bg-emerald-50 text-emerald-700" : status === "loading" ? "bg-slate-100 text-slate-600" : "bg-amber-50 text-amber-700"}`}>{status === "online" ? "Data online" : status === "loading" ? "Loading" : "Data unavailable"}</span>}
    >
      {status === "offline" ? <p className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">{message ?? "Incentives backend unavailable."}</p> : null}
      <div className="grid gap-4 lg:grid-cols-4">
        <ScoreBlock label="Latest Net Revenue" value={usdm(latest?.netRevenue)} note={latest?.label ?? "n/a"} />
        <ScoreBlock label="Latest Incentives" value={usdm(latest?.rebatesIncentives)} note="Client incentives / rebates" />
        <ScoreBlock label="Latest Incentive Ratio" value={pct(latest?.incentivesToNetRevenue)} note="Incentives / net revenue" />
        <ScoreBlock label="Avg Incentive Ratio" value={pct(history?.summary.averageIncentivesToNetRevenue)} note={`${rows.length} quarters`} />
      </div>
      <div className="mt-5">
        <ChartPanel title="Quarterly Incentives vs Net Revenue">
              ...
              <Tooltip
                formatter={(value: number, name: string) => {
                  const labels: Record<string, string> = {
                    ...
                    rebatesIncentives: "Incentives / rebates",
                  };
                  return [usdm(value), labels[name] ?? name];
                }}
                labelFormatter={(label, payload) => {
                  const row = payload?.[0]?.payload as MaIncentivesNetRevenueRow | undefined;
                  return row ? `${label} / ${row.asOfDate ?? "n/a"} / incentives ${pct(row.incentivesToNetRevenue)}` : String(label);
                }}
              />
              <Legend />
              <Bar dataKey="netRevenue" fill="#2563eb" name="Net revenue" />
              <Bar dataKey="rebatesIncentives" fill="#0f766e" name="Incentives / rebates" />
            </BarChart>
          </ResponsiveContainer>
        </ChartPanel>
      </div>
      <div className="mt-5">
        <ChartPanel title="Incentives / Net Revenue Trend">
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={chartRows}>
              ...
              <Tooltip
                formatter={(value: number) => [`${value.toFixed(1)}%`, "Incentives / net revenue"]}
                labelFormatter={(label, payload) => {
                  const row = payload?.[0]?.payload as MaIncentivesNetRevenueRow | undefined;
                  return row ? `${label} / ${row.asOfDate ?? "n/a"} / incentives ${usdm(row.rebatesIncentives)} / net revenue ${usdm(row.netRevenue)}` : String(label);
                }}
              />
              <Legend />
              <Line type="monotone" dataKey="incentiveRatioPct" dot={false} stroke="#a16207" strokeWidth={2.5} name="Incentives / net revenue" />
            </LineChart>
          </ResponsiveContainer>
        </ChartPanel>
      </div>
      <div className="mt-5">
        <ChartPanel title="QoQ Growth: Incentives vs Net Revenue">
          <ResponsiveContainer width="100%" height={320}>
            <LineChart data={growthRows}>
              <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
              <XAxis dataKey="label" interval={chartTickInterval(growthRows.length)} tick={{ fontSize: 11 }} />
              <YAxis tickFormatter={(value: number) => `${Number(value).toFixed(0)}%`} />
              <Tooltip
                formatter={(value: number, name: string) => {
                  const labels: Record<string, string> = {
                    netRevenueGrowth: "Net revenue QoQ",
                    rebatesIncentivesGrowth: "Incentives / rebates QoQ",
                  };
                  return [`${value.toFixed(1)}%`, labels[name] ?? name];
                }}
                labelFormatter={(label, payload) => {
                  const row = payload?.[0]?.payload as (MaIncentivesNetRevenueRow & { netRevenueGrowth?: number | null; rebatesIncentivesGrowth?: number | null }) | undefined;
                  return row ? `${label} / ${row.asOfDate ?? "n/a"} / revenue growth ${row.netRevenueGrowth == null ? "n/a" : `${row.netRevenueGrowth.toFixed(1)}%`} / incentives growth ${row.rebatesIncentivesGrowth == null ? "n/a" : `${row.rebatesIncentivesGrowth.toFixed(1)}%`}` : String(label);
                }}
              />
              <Legend />
              <Line type="monotone" dataKey="netRevenueGrowth" dot={false} stroke="#2563eb" strokeWidth={2.5} name="Net revenue QoQ" />
              <Line type="monotone" dataKey="rebatesIncentivesGrowth" dot={false} stroke="#0f766e" strokeWidth={2.5} name="Incentives / rebates QoQ" />
            </LineChart>
          </ResponsiveContainer>
        </ChartPanel>
      </div>
      <div className="mt-5">
        <ChartPanel title="YoY Growth: Incentives vs Net Revenue">
          <ResponsiveContainer width="100%" height={320}>
            <BarChart data={yoyRows}>
              <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
              <XAxis dataKey="label" interval={chartTickInterval(yoyRows.length)} tick={{ fontSize: 11 }} />
              <YAxis tickFormatter={(value: number) => `${Number(value).toFixed(0)}%`} />
              <Tooltip
                formatter={(value: number, name: string) => {
                  const labels: Record<string, string> = {
                    netRevenueYoyGrowth: "Net revenue YoY",
                    rebatesIncentivesYoyGrowth: "Incentives / rebates YoY",
                  };
                  return [`${value.toFixed(1)}%`, labels[name] ?? name];
                }}
                labelFormatter={(label, payload) => {
                  const row = payload?.[0]?.payload as (MaIncentivesNetRevenueRow & { netRevenueYoyGrowth?: number | null; rebatesIncentivesYoyGrowth?: number | null }) | undefined;
                  return row ? `${label} / ${row.asOfDate ?? "n/a"} / revenue YoY ${row.netRevenueYoyGrowth == null ? "n/a" : `${row.netRevenueYoyGrowth.toFixed(1)}%`} / incentives YoY ${row.rebatesIncentivesYoyGrowth == null ? "n/a" : `${row.rebatesIncentivesYoyGrowth.toFixed(1)}%`}` : String(label);
                }}
              />
              <Legend />
              <Bar dataKey="netRevenueYoyGrowth" fill="#2563eb" name="Net revenue YoY" />
              <Bar dataKey="rebatesIncentivesYoyGrowth" fill="#0f766e" name="Incentives / rebates YoY" />
            </BarChart>
          </ResponsiveContainer>
        </ChartPanel>
      </div>
      {warningText ? <p className="mt-4 rounded-lg bg-amber-50 p-3 text-sm text-amber-900">{warningText}</p> : null}
      <div className="mt-5">
        <DataTable
          headers={["Quarter", "Event Date", "Net Revenue", "Incentives", "Incentives / Revenue", "Source"]}
          rows={rows.map((row) => [
            row.label,
            row.asOfDate ?? "n/a",
            usdm(row.netRevenue),
            usdm(row.rebatesIncentives),
            pct(row.incentivesToNetRevenue),
            row.sourceQuality,
          ])}
        />
      </div>
...
function MaCapitalReturnsBackendPanel() {
  const [history, setHistory] = useState<MaCapitalReturnResponse | null>(null);
  const [status, setStatus] = useState<"loading" | "online" | "offline">("loading");
  const [message, setMessage] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    async function loadHistory() {
      try {
        const payload = await fetchJsonWithFallback<MaCapitalReturnResponse>([
          "/api/ma/capital-returns?years=8",
          "/api/stocks/ma/capital-returns?years=8",
        ]);
        if (!cancelled) {
          setHistory(payload);
          setStatus("online");
  ...
  return (
    <SectionCard
      ...
      description="Eight-year MA capital-return history from backend financial-period data. Dividends and buybacks stack into one capital-return bar and are compared with annual FCF."
      badge={<span className={`rounded-full px-3 py-1 text-xs font-semibold uppercase ${status === "online" ? "bg-emerald-50 text-emerald-700" : status === "loading" ? "bg-slate-100 text-slate-600" : "bg-amber-50 text-amber-700"}`}>{status === "online" ? "Data online" : status === "loading" ? "Loading" : "Data unavailable"}</span>}
    >
      ...
      <div className="mt-5">
        <ChartPanel title="Capital Return Stack vs FCF">
          <ResponsiveContainer width="100%" height={360}>
            <BarChart data={chartRows}>
              <defs>
                <pattern id="maDividendForecastHatch" width="8" height="8" patternUnits="userSpaceOnUse" patternTransform="rotate(45)">
                  <rect width="8" height="8" fill="#ecfdf5" />
                  <path d="M 0 0 L 0 8" stroke="#0f766e" strokeWidth="2" />
                </pattern>
                <pattern id="maBuybackForecastHatch" width="8" height="8" patternUnits="userSpaceOnUse" patternTransform="rotate(45)">
                  <rect width="8" height="8" fill="#eff6ff" />
                  <path d="M 0 0 L 0 8" stroke="#2563eb" strokeWidth="2" />
                </pattern>
                <pattern id="maFcfForecastHatch" width="8" height="8" patternUnits="userSpaceOnUse" patternTransform="rotate(45)">
              ...
              <Tooltip
                ...
                labelFormatter={(label, payload) => {
                  const row = payload?.[0]?.payload as (MaCapitalReturnRow & { rawJson?: { forecastAssumptionLabel?: string }; label?: string; equityFreeCashFlowForecast?: number | null }) | undefined;
              ...
              <Bar dataKey="dividendCashForecast" stackId="forecastCapitalReturn" fill="url(#maDividendForecastHatch)" stroke="#0f766e" strokeDasharray="4 3" name="Forecast dividends" />
              <Bar dataKey="buybackForecast" stackId="forecastCapitalReturn" fill="url(#maBuybackForecastHatch)" stroke="#2563eb" strokeDasharray="4 3" name="Forecast buybacks" />
              <Bar dataKey="equityFreeCashFlowForecast" fill="url(#maFcfForecastHatch)" stroke="#f97316" strokeDasharray="4 3" name="Forecast FCF" />
            </BarChart>
          </ResponsiveContainer>
        </ChartPanel>
      </div>
...
function MaBacktestPanel() {
  const [startDate, setStartDate] = useState("2018-01-02");
  const [endDate, setEndDate] = useState("2026-05-12");
  const [status, setStatus] = useState<"idle" | "running" | "done" | "error">("idle");
  const [result, setResult] = useState<MaBacktestResult | null>(null);

  const runBacktest = useCallback(async () => {
    setStatus("running");
    try {
      const payload = await fetchJsonWithFallback<MaBacktestResult>([
        "/api/ma/backtests",
        "/api/stocks/ma/backtests",
      ], {
        method: "POST",
  ...
  return (
    <SectionCard
      title="MA vs SPY Backtest"
      description="Select a date range and compare daily MA buy-and-hold performance against SPY from backend price history."
          ...
          <ChartPanel title="Total Return Since Start">
            <ResponsiveContainer width="100%" height={320}>
              <LineChart data={curve}>
                ...
                <Tooltip formatter={(value: number) => `${value.toFixed(1)}%`} />
                <Legend />
                <Line type="monotone" dataKey="maReturn" dot={false} stroke="#2563eb" strokeWidth={2.5} name="MA" />
                <Line type="monotone" dataKey="spyReturn" dot={false} stroke="#64748b" strokeWidth={2.5} name="SPY" />
              </LineChart>
            </ResponsiveContainer>
          </ChartPanel>
          <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <ScoreBlock label="MA CAGR" value={pct(metrics.maBuyHold?.cagr)} note="Buy-and-hold" />
            <ScoreBlock label="SPY CAGR" value={pct(metrics.spy?.cagr)} note="Benchmark" />
            <ScoreBlock label="MA MDD" value={pct(metrics.maBuyHold?.maxDrawdown)} note="Maximum drawdown" />
            <ScoreBlock label="SPY MDD" value={pct(metrics.spy?.maxDrawdown)} note="Maximum drawdown" />
            <ScoreBlock label="MA Sharpe" value={metrics.maBuyHold?.sharpe != null ? metrics.maBuyHold.sharpe.toFixed(2) : "n/a"} note="Zero risk-free rate" />
            <ScoreBlock label="SPY Sharpe" value={metrics.spy?.sharpe != null ? metrics.spy.sharpe.toFixed(2) : "n/a"} note="Zero risk-free rate" />
            <ScoreBlock label="MA Vol" value={pct(metrics.maBuyHold?.volatility)} note="Annualized daily vol" />
...
function MaValuationTriangulationPanel({ valuation }: { valuation: ReturnType<typeof buildMaDashboardData>["valuation"] }) {
  const methodRows = valuation.methodCards.map((card) => ({
    method: card.label,
    value: card.value,
  }));
  return (
    <SectionCard title="MA Valuation Triangulation / Method Outputs" description="DCF / FCFF, FCF yield, P/E, EV/EBIT, and payments peer-premium methods test whether MA deserves a premium multiple as growth normalizes.">
      <div className="grid gap-6 xl:grid-cols-2">
