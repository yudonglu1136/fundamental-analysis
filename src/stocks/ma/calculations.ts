...
import { maScenarioPresets, defaultMaValuationAssumptions } from "./assumptions";
import { maDataset } from "./data";
import type { MaDataset, MaFinancialPeriod, MaValuationAssumptions } from "./model";
...
function latestPeriod(data: MaDataset) {
...
function annualizePeriod(period: MaFinancialPeriod) {
...
function latestMetric(data: MaDataset) {
...
export function getMaPeriods() {
  return maDataset.periods.map((period) => ({ value: period.id, label: period.label }));
}

export function getDefaultMaPeriod() {
  return maDataset.periods[maDataset.periods.length - 1]?.id ?? "fy2026-q1";
}

export function calculateMaSummary(data: unknown): SummaryMetric[] {
  const dataset = (data as MaDataset) ?? maDataset;
...
export function calculateMaValuation(
  data: unknown,
  assumptionOverrides: Partial<MaValuationAssumptions> = {},
  ...
  const annualized = annualizePeriod(period);
  const metric = latestMetric(dataset);
  const preset = maScenarioPresets[scenario] ?? maScenarioPresets.Base;
  const assumptions: MaValuationAssumptions = {
    ...defaultMaValuationAssumptions,
    ...preset,
    ...assumptionOverrides,
  };
  function computePoint(name: Scenario, driverSet: MaValuationAssumptions) {
  ...
  const fairValues = (["Bear", "Base", "Bull"] as Scenario[]).map((name) =>
    computePoint(name, { ...defaultMaValuationAssumptions, ...maScenarioPresets[name] }),
  );
  ...
  return {
    ...
    methodCards: [
      ...
      methodCard("peer-premium", "Payments Peer Premium", peer, "Visa/Amex/processor guardrail with explicit premium or discount."),
    ],
    ...
    dcfValue: dcf,
    validationWarnings: [
      buildValidationWarning("ma-static-fallback", "Static fallback", "Interactive valuation uses local fallback until backend saved runs are inspected."),
    ],
  };
}

export function buildMaDashboardData(
  data: MaDataset,
  scenario: Scenario,
  assumptions: Partial<MaValuationAssumptions>,
) {
  const period = latestPeriod(data);
  const annualized = annualizePeriod(period);
  const metric = latestMetric(data);
  const valuation = calculateMaValuation(data, assumptions, scenario);
...
export function resolveMaDataset(data: unknown): MaDataset {
  return (data as MaDataset) ?? maDataset;
}
