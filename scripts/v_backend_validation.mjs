...
import { runVBacktest } from "../apps/api/src/services/vBacktestService.mjs";
import { getVCapitalReturnHistory } from "../apps/api/src/services/vSnapshotService.mjs";
import { createVValuationRun } from "../apps/api/src/services/vValuationService.mjs";
import { V_BACKEND_DB_PATH, V_BACKEND_TABLES } from "../modules/v/db/schema.mjs";
import { V_BACKEND_MODEL_VERSION } from "../modules/v/valuation/modelVersion.mjs";

const TICKER = "V";
const MODEL_VERSION = V_BACKEND_MODEL_VERSION.version;
const checks = [];
...
function count(table, where = "ticker = 'V'") {
  return query(`SELECT COUNT(*) AS count FROM ${table} WHERE ${where}`, [], V_BACKEND_DB_PATH)[0]?.count ?? 0;
}
...
async function main() {
  console.log("V Backend Validation");

  if (!existsSync(V_BACKEND_DB_PATH)) {
    fail("DB file exists", `${V_BACKEND_DB_PATH} is missing. Run npm run v:backend:seed.`);
    printAndExit();
    return;
  }
  pass("DB file exists", V_BACKEND_DB_PATH);

  const tables = new Set(query("SELECT name FROM sqlite_master WHERE type = 'table'", [], V_BACKEND_DB_PATH).map((row) => row.name));
  for (const table of V_BACKEND_TABLES) {
    tables.has(table) ? pass(`Table exists: ${table}`) : fail(`Table missing: ${table}`, "Run npm run v:backend:seed.");
  }
  ...
  eventCount >= 34 ? pass("At least 8 years of quarterly reporting events imported", `${eventCount} rows`) : fail("At least 8 years of quarterly reporting events imported", `${eventCount} rows`);
  ...
  missingPeriods.length === 0
    ? pass("Quarterly history covers FY2018 Q1 through FY2026 Q2", `${expectedFiscalPeriods().length} expected periods`)
    : fail("Quarterly history covers FY2018 Q1 through FY2026 Q2", `Missing: ${missingPeriods.join(", ")}`);
  ...
  metricRows.length >= eventCount && metricRows.every((row) => isFiniteNumber(row.switchedTransactions) && isFiniteNumber(row.grossDollarVolume) && isFiniteNumber(row.takeRate))
    ? pass("V-specific payments metrics imported", `${metricRows.length} rows with GDV, purchase volume, cross-border and switched transaction fields`)
    : fail("V-specific payments metrics imported", `${metricRows.length} rows; missing V-specific fields.`);

  const modelVersionCount = count("model_versions");
  modelVersionCount > 0 ? pass("Model versions imported", `${modelVersionCount} rows`) : fail("Model versions imported", "No V model version rows found.");
  ...
  const latestEvent = query("SELECT * FROM reporting_events WHERE ticker = ? ORDER BY eventDate DESC LIMIT 1", [TICKER], V_BACKEND_DB_PATH)[0];
  if (!latestEvent) {
    ...
    try {
      const valuation = await createVValuationRun({ eventId: latestEvent.id, scenario: "Base", modelVersion: MODEL_VERSION });
      ...
      if (createdValuationId) {
        execute("DELETE FROM valuation_runs WHERE id = ?", [createdValuationId], V_BACKEND_DB_PATH);
      }
    }
  }
  ...
  eventCount > 0 && historicalRunEventIds.size >= eventCount
    ...
    : fail("Historical Base valuations exist for each event", `${historicalRunEventIds.size}/${eventCount} events. Run npm run v:backend:backfill-valuations -- --base-only.`);
  ...
  const oldYearLeakage = earlyRuns.filter((row) => {
    ...
    return (
      ...
      (snapshot.vAnalyticalFramework?.takeRate ?? 0) > 0.004
    );
  });
  earlyRuns.length >= 4 && oldYearLeakage.length === 0
    ? pass("FY2018 valuations do not leak latest V scale or current risk assumptions")
    : fail("FY2018 valuations do not leak latest V scale or current risk assumptions", JSON.stringify(oldYearLeakage.map((row) => row.id)));
  ...
  const transcriptModelReady = count("transcript_extractions", "ticker = 'V' AND (modelReady != 0 OR valuationImpactAllowed != 0)");
  ...
  const guidanceCandidatesPromoted = count(
    "guidance_items",
    "ticker = 'V' AND valuationImpactAllowed != 0 AND (guidanceType = 'candidate' OR humanReviewStatus IN ('needs_review', 'unreviewed'))",
  );
  ...
  const priceBars = query(
    "SELECT ticker, COUNT(*) AS count, MIN(priceDate) AS firstDate, MAX(priceDate) AS lastDate FROM daily_price_bars WHERE ticker IN ('V', 'SPY') GROUP BY ticker",
    [],
    V_BACKEND_DB_PATH,
  );
  const priceBarMap = new Map(priceBars.map((row) => [row.ticker, row]));
  const vPriceBars = priceBarMap.get("V");
  const spyPriceBars = priceBarMap.get("SPY");
  vPriceBars?.count >= 2000
    ? pass("V daily price bars exist", `${vPriceBars.count} rows, ${vPriceBars.firstDate} to ${vPriceBars.lastDate}`)
    : fail("V daily price bars exist", "Run npm run v:backend:import-prices.");
  spyPriceBars?.count >= 2000
    ...
    : fail("SPY daily price bars exist", "Run npm run v:backend:import-prices.");

  try {
    const backtest = runVBacktest({ startDate: "2018-01-02", endDate: "2026-05-12", benchmarkTicker: "SPY" });
    const metrics = backtest.metrics ?? {};
    backtest.status === "completed" &&
      backtest.curve?.length >= 2000 &&
      isFiniteNumber(metrics.vBuyHold?.cagr) &&
      isFiniteNumber(metrics.vBuyHold?.maxDrawdown) &&
      isFiniteNumber(metrics.vBuyHold?.sharpe) &&
      isFiniteNumber(metrics.vBuyHold?.volatility) &&
      ...
      ? pass("V vs SPY backtest returns finite metrics", `curve=${backtest.curve.length}; vagr=${metrics.vBuyHold.cagr}; spyCagr=${metrics.spy.cagr}`)
      : fail("V vs SPY backtest returns finite metrics", JSON.stringify(backtest).slice(0, 1200));
  } catch (error) {
    fail("V vs SPY backtest returns finite metrics", error instanceof Error ? error.message : String(error));
  }

  try {
    const legacyRoute = await checkCapitalReturnRoute("/api/v/capital-returns?years=8");
    const unifiedRoute = await checkCapitalReturnRoute("/api/stocks/v/capital-returns?years=8");
    ...
    const capitalReturns = getVCapitalReturnHistory({ years: 8 });
    ...
    rows.length === 8 && completeRows.length === 8
      ? pass("V capital-return service has 8 annual rows", `${rows[0]?.fiscalYear}-${rows[rows.length - 1]?.fiscalYear}`)
      : fail("V capital-return service has 8 annual rows", JSON.stringify({ rows: rows.length, completeRows: completeRows.length, warnings: capitalReturns.warnings }));
    ...
    hasStackedCapitalReturnSeries
      ? pass("V stacked capital-return series exists", `cumulativeDividend=${capitalReturns.summary.cumulativeDividendCash}; cumulativeBuybacks=${capitalReturns.summary.cumulativeBuybacks}`)
      : fail("V stacked capital-return series exists", JSON.stringify(capitalReturns.summary));
    ...
    hasFcfComparisonSeries
      ? pass("V FCF comparison series exists", `cumulativeFcf=${capitalReturns.summary.cumulativeFcf}`)
      : fail("V FCF comparison series exists", JSON.stringify(rows.map((row) => ({ fiscalYear: row.fiscalYear, fcf: row.equityFreeCashFlow }))));
    ...
    forward?.isForecast === true &&
      ...
      ? pass("V capital-return history includes forward forecast bar", `FY${forward.fiscalYear}E; capitalReturn=${forward.totalCapitalReturn}`)
      : fail("V capital-return history includes forward forecast bar", JSON.stringify(forward));
    ...
    const proxyWarning = capitalReturns.warnings?.some((warning) => warning.id === "v-capital-return-proxy-years");
    proxyRows.length === 0 || proxyWarning
      ? pass("V proxy capital-return rows generate warnings", proxyRows.length ? `Proxy/seed years: ${proxyRows.map((row) => row.fiscalYear).join(", ")}` : "No proxy rows")
      : fail("V proxy capital-return rows generate warnings", JSON.stringify(proxyRows));
    ...
    frontendFieldsPresent
      ? pass("V capital-return frontend-required fields are present")
      : fail("V capital-return frontend-required fields are present", JSON.stringify([...rows, forward].filter(Boolean)[0]));
    ...
    notCollapsedRows.length === rows.length && rows.some((row) => (row.buybackAmount ?? 0) > (row.dividendCashCost ?? 0) * 3)
      ? pass("V buybacks and dividends are not collapsed together", `${notCollapsedRows.length}/8 rows retain separate dividend and buyback fields`)
      : fail("V buybacks and dividends are not collapsed together", JSON.stringify(rows.map((row) => ({ fiscalYear: row.fiscalYear, dividends: row.dividendCashCost, buybacks: row.buybackAmount, total: row.totalCapitalReturn }))));
    ...
    coverageRows.every((row) => isFiniteNumber(row.fcfCoverage))
      ? pass("V FCF coverage is finite when capital return exceeds FCF", coverageRows.length ? coverageRows.map((row) => `FY${row.fiscalYear}:${row.fcfCoverage}`).join(", ") : "No coverage-pressure years")
      : fail("V FCF coverage is finite when capital return exceeds FCF", JSON.stringify(coverageRows));
  } catch (error) {
    fail("V capital-return endpoint/service works", error instanceof Error ? error.message : String(error));
  }

  try {
    const legacyRoute = await routeStockBackend({ method: "GET" }, new URL("http://127.0.0.1:8787/api/v/incentives-vs-net-revenue?quarters=40"), null);
    const unifiedRoute = await routeStockBackend({ method: "GET" }, new URL("http://127.0.0.1:8787/api/stocks/v/incentives-vs-net-revenue?quarters=40"), null);
    ...
    legacyRoute?.status === 200 && unifiedRoute?.status === 200 && rows.length >= 34 && completeRows.length === rows.length
      ? pass("V incentives-vs-net-revenue endpoints work", `${rows.length} quarterly rows`)
      : fail("V incentives-vs-net-revenue endpoints work", JSON.stringify({ legacy: legacyRoute?.status, unified: unifiedRoute?.status, rows: rows.length, completeRows: completeRows.length }));
  } catch (error) {
    fail("V incentives-vs-net-revenue endpoints work", error instanceof Error ? error.message : String(error));
  }
