...
const TRANSCRIPT_DIR = path.join(ROOT, "data/local/isrg/transcripts");
...
function countTopicMentions(text) {
  ...
  return TOPICS.map((topic) => ({
    topic,
    mentions: (normalized.match(new RegExp(topic.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "g")) ?? []).length,
  })).filter((item) => item.mentions > 0);
}

const manifest = await readJson(MANIFEST_PATH, { ticker: "ISRG", records: [] });
...
for (const file of rawFiles) {
  ...
  topicRows.push({
    ...
    notes: "Auto topic counts are research-only and require analyst review.",
  });
...
const intelligence = {
  ...
  sourceDiscipline:
    "Transcript intelligence is research-only. Numeric claims are candidates until validated against official releases, 10-Q/10-K, or 8-K exhibits.",
...
await fs.writeFile(
  ...
  JSON.stringify({ ticker: "ISRG", builtAt: intelligence.builtAt, rows: topicRows }, null, 2),
);

console.log(`ISRG transcript intelligence built from ${rawFiles.length} raw files.`);
