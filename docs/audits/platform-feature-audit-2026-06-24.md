...
| Valuation matrix | Search and rank tickers | Find valuation candidates and filter by ticker | `ValuationCompactDashboard`, `ValuationTickerPickerCard` | Search exists | Entire valuation payload can be large | P0 | Split list summary from ticker detail; cache details by ticker | Open |
...
| Performance | Page load and long-lived sessions | Avoid browser refresh to recover | `ApiClient`, `TerminalHome`, `SecondaryDashboard` | Loading/error states now include in-page recovery for empty secondary screens | API calls now time out; idempotent GETs retry once; top-level stale responses ignored | P0 | Add per-module recovery and data freshness telemetry | First pass fixed |
