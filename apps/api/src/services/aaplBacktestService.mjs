...
export function getAaplBacktests() {
  return query(
...
export function runAaplBacktest({
  startDate = "2018-01-02",
  endDate = "2026-05-12",
  modelVersion = DEFAULT_MODEL_VERSION,
  benchmarkTicker = "SPY",
} = {}) {
  const aaplBars = getPriceBars(TICKER, startDate, endDate);
  const benchmarkBars = getPriceBars(benchmarkTicker, startDate, endDate);
  const warnings = [];
  if (aaplBars.length < 2) warnings.push("AAPL daily adjusted price history is unavailable or too short for the selected window.");
  ...
  let aaplBuyHoldValue = 1;
  let benchmarkValue = 1;
  let previousAapl = aaplBars[0];
  let previousBenchmark = benchmarkByDate.get(aaplBars[0].priceDate) ?? benchmarkBars[0];
  const curve = [];

  for (let index = 0; index < aaplBars.length; index += 1) {
    const aapl = aaplBars[index];
    const benchmark = benchmarkByDate.get(aapl.priceDate);
    if (!benchmark) continue;
    if (curve.length) {
      const aaplReturn = aapl.adjustedClose / previousAapl.adjustedClose - 1;
      const benchmarkReturn = benchmark.adjustedClose / previousBenchmark.adjustedClose - 1;
      aaplBuyHoldValue *= 1 + aaplReturn;
      benchmarkValue *= 1 + benchmarkReturn;
    }
    curve.push({
      date: aapl.priceDate,
      aaplBuyHold: aaplBuyHoldValue,
      spy: benchmarkValue,
      benchmark: benchmarkValue,
      price: aapl.adjustedClose,
    });
    previousAapl = aapl;
  ...
  const result = {
    ...
    priceBars: {
      ...
      [benchmarkTicker]: benchmarkBars.length,
      sources: {
        [TICKER]: aaplBars[0]?.source ?? null,
        [benchmarkTicker]: benchmarkBars[0]?.source ?? null,
      },
    },
    metrics: {
      aaplBuyHold: metrics(curve.map((row) => ({ date: row.date, value: row.aaplBuyHold }))),
