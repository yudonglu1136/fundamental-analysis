...
export function getMaBacktests() {
  return query(
...
export function runMaBacktest({
  ...
  const maBars = getPriceBars(TICKER, startDate, endDate);
  const benchmarkBars = getPriceBars(benchmarkTicker, startDate, endDate);
  const warnings = [];
  if (maBars.length < 2) warnings.push("MA daily adjusted price history is unavailable or too short for the selected window.");
  ...
  let maBuyHoldValue = 1;
  let benchmarkValue = 1;
  let previousMa = maBars[0];
  let previousBenchmark = benchmarkByDate.get(maBars[0].priceDate) ?? benchmarkBars[0];
  const curve = [];

  for (let index = 0; index < maBars.length; index += 1) {
    const ma = maBars[index];
    const benchmark = benchmarkByDate.get(ma.priceDate);
    if (!benchmark) continue;
    if (curve.length) {
      const maReturn = ma.adjustedClose / previousMa.adjustedClose - 1;
      const benchmarkReturn = benchmark.adjustedClose / previousBenchmark.adjustedClose - 1;
      maBuyHoldValue *= 1 + maReturn;
      benchmarkValue *= 1 + benchmarkReturn;
    }
    curve.push({
      date: ma.priceDate,
      maBuyHold: maBuyHoldValue,
      spy: benchmarkValue,
      benchmark: benchmarkValue,
      price: ma.adjustedClose,
    });
    previousMa = ma;
  ...
  const result = {
    ...
    priceBars: {
      ...
      [benchmarkTicker]: benchmarkBars.length,
      sources: {
        [TICKER]: maBars[0]?.source ?? null,
        [benchmarkTicker]: benchmarkBars[0]?.source ?? null,
      },
    },
    metrics: {
      maBuyHold: metrics(curve.map((row) => ({ date: row.date, value: row.maBuyHold }))),
