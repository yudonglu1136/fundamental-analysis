...
function getMaDailyPriceOnOrBefore(asOfDate) {
...
export function getMaReportingEvents() {
  return query("SELECT * FROM reporting_events WHERE ticker = ? ORDER BY eventDate DESC", [TICKER], MA_BACKEND_DB_PATH);
}

export function getLatestMaEvent() {
  return getMaReportingEvents()[0] ?? null;
}

export function resolveMaEvent({ eventId, asOfDate } = {}) {
  if (eventId) {
    return query("SELECT * FROM reporting_events WHERE ticker = ? AND id = ? LIMIT 1", [TICKER, eventId], MA_BACKEND_DB_PATH)[0] ?? null;
  }
  if (asOfDate) {
    return query(
  ...
  return getLatestMaEvent();
}

export function getMaSnapshot({ eventId, asOfDate } = {}) {
  const reportingEvent = resolveMaEvent({ eventId, asOfDate });
  ...
  const dailyPrice = getMaDailyPriceOnOrBefore(effectiveAsOfDate);
...
export function getMaIncentivesVsNetRevenueHistory({ quarters = 40 } = {}) {
  ...
  return {
    ...
    warnings: proxyRows.length
      ? [{
          id: "ma-incentives-proxy-rows",
          severity: "medium",
          title: "Some MA incentive rows use proxy/seed data",
...
function buildMaAnnualCapitalReturnRows(limit) {
  ...
  return fiscalYears.map((fiscalYear) => {
    ...
    return {
      ...
      isForecast: false,
      rawJson: {
        source: "MA backend financial_periods annual aggregation",
...
function buildMaForwardCapitalReturnExpectation(latestActual) {
  ...
  return {
    ...
    rawJson: {
      ...
      dividendSource: "Latest annual DPS grown 12% to reflect MA's low payout and dividend growth history.",
...
export function getMaCapitalReturnHistory({ years = 8 } = {}) {
  const limit = Math.max(1, Math.min(Number(years) || 8, 12));
  const rows = buildMaAnnualCapitalReturnRows(limit);
  const latest = rows[rows.length - 1] ?? null;
  const forwardExpectation = buildMaForwardCapitalReturnExpectation(latest);
  ...
  if (proxyYears.length) {
    warnings.push({
      id: "ma-capital-return-proxy-years",
      severity: "medium",
      title: "Some MA capital-return rows use seed/proxy data",
  ...
  if (capitalReturnExceedsFcfYears.length) {
    warnings.push({
      id: "ma-capital-return-fcf-coverage-pressure",
      severity: "low",
