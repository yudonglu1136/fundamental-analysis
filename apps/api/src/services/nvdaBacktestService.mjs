...
export function getNvdaBacktests() {
  return query(
...
export function runNvdaBacktest({
  startDate = "2018-01-02",
  endDate = "2026-05-12",
  modelVersion = DEFAULT_MODEL_VERSION,
  benchmarkTicker = "SPY",
} = {}) {
  const nvdaBars = getPriceBars(TICKER, startDate, endDate);
  const benchmarkBars = getPriceBars(benchmarkTicker, startDate, endDate);
  const warnings = [];
  if (nvdaBars.length < 2) warnings.push("NVDA daily price history is unavailable or too short for the selected window.");
  ...
  if (warnings.length) {
    return {
      ...
      warnings,
      priceBars: {
        [TICKER]: nvdaBars.length,
        [benchmarkTicker]: benchmarkBars.length,
      },
    };
  }

  if (nvdaBars.some((row) => String(row.sourceType ?? "").includes("unadjusted")) || benchmarkBars.some((row) => String(row.sourceType ?? "").includes("unadjusted"))) {
    warnings.push("At least one price source uses close as adjustedClose because adjusted close was unavailable.");
  }
  if (nvdaBars.some((row) => String(row.sourceType ?? "").includes("proxy")) || benchmarkBars.some((row) => String(row.sourceType ?? "").includes("proxy"))) {
    warnings.push("At least one price source uses research-only proxy price bars.");
  }

  const benchmarkByDate = new Map(benchmarkBars.map((row) => [row.priceDate, row]));
  let nvdaBuyHoldValue = 1;
  let benchmarkValue = 1;
  let previousNvda = nvdaBars[0];
  let previousBenchmark = benchmarkByDate.get(nvdaBars[0].priceDate) ?? benchmarkBars[0];
  const curve = [];

  for (let index = 0; index < nvdaBars.length; index += 1) {
    const nvda = nvdaBars[index];
    const benchmark = benchmarkByDate.get(nvda.priceDate);
    if (!benchmark) continue;
    if (curve.length) {
      const nvdaReturn = nvda.adjustedClose / previousNvda.adjustedClose - 1;
      const benchmarkReturn = benchmark.adjustedClose / previousBenchmark.adjustedClose - 1;
      nvdaBuyHoldValue *= 1 + nvdaReturn;
  ...
  const result = {
    ...
    metrics: {
      ...
      nvdaBuyHold: metrics(curve.map((row) => ({ date: row.date, value: row.nvdaBuyHold }))),
    },
    curve,
    warnings,
  };
