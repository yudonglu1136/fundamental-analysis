...
function getNowDailyPriceOnOrBefore(asOfDate) {
...
export function getNowReportingEvents() {
  return query("SELECT * FROM reporting_events WHERE ticker = ? ORDER BY eventDate DESC", [TICKER], NOW_BACKEND_DB_PATH);
}

export function getLatestNowEvent() {
  return getNowReportingEvents()[0] ?? null;
}

export function resolveNowEvent({ eventId, asOfDate } = {}) {
  if (eventId) {
    return query("SELECT * FROM reporting_events WHERE ticker = ? AND id = ? LIMIT 1", [TICKER, eventId], NOW_BACKEND_DB_PATH)[0] ?? null;
  }
  if (asOfDate) {
    return query(
  ...
  return getLatestNowEvent();
}

export function getNowSnapshot({ eventId, asOfDate } = {}) {
  const reportingEvent = resolveNowEvent({ eventId, asOfDate });
  ...
  const dailyPrice = getNowDailyPriceOnOrBefore(effectiveAsOfDate);
...
export function getNowSubscriptionAgentHistory({ quarters = 40 } = {}) {
  ...
  return {
    ...
    warnings: proxyRows.length
      ? [{
          id: "now-subscription-agent-proxy-rows",
          severity: "medium",
          title: "Some NOW subscription/Agent rows use proxy/seed data",
...
function buildNowAnnualCapitalReturnRows(limit) {
  ...
  return fiscalYears.map((fiscalYear) => {
    ...
    return {
      ...
      isForecast: false,
      rawJson: {
        source: "NOW backend financial_periods annual aggregation",
        coverageTreatment: "FCF coverage uses equity FCF divided by gross share repurchases. Dividends are zero unless ServiceNow starts paying one.",
...
function buildNowForwardCapitalReturnExpectation(latestActual) {
  ...
  return {
    ...
    rawJson: {
      ...
      dividendSource: "NOW has no dividend in this seed framework; DPS remains zero.",
...
export function getNowCapitalReturnHistory({ years = 8 } = {}) {
  const limit = Math.max(1, Math.min(Number(years) || 8, 12));
  const rows = buildNowAnnualCapitalReturnRows(limit);
  const latest = rows[rows.length - 1] ?? null;
  const forwardExpectation = buildNowForwardCapitalReturnExpectation(latest);
  ...
  if (proxyYears.length) {
    warnings.push({
      id: "now-capital-return-proxy-years",
      severity: "medium",
      title: "Some NOW capital-return rows use seed/proxy data",
  ...
  if (capitalReturnExceedsFcfYears.length) {
    warnings.push({
      id: "now-capital-return-fcf-coverage-pressure",
      severity: "low",
