...
import { getMaReportingEvents, getMaSnapshot } from "./maSnapshotService.mjs";
...
const TICKER = "MA";
const DEFAULT_MODEL_VERSION = MA_BACKEND_MODEL_VERSION.version;
...
function applyDailyPriceToSnapshot(snapshot) {
  ...
  return {
    ...
    snapshot: {
      ...
      marketSnapshot: {
        ...marketSnapshot,
        id: marketSnapshot.id ?? `ma-daily-price-${snapshot.asOfDate}`,
        ticker: TICKER,
...
export function getMaValuationRuns({ asOfDate, eventId, scenario, modelVersion } = {}) {
  ...
  return query(`SELECT * FROM valuation_runs WHERE ${clauses.join(" AND ")} ORDER BY createdAt DESC`, params, MA_BACKEND_DB_PATH).map((row) => ({
    ...row,
...
export function getMaHistoricalValuations({ scenario = "Base", modelVersion = DEFAULT_MODEL_VERSION } = {}) {
  const events = getMaReportingEvents();
  const runs = getMaValuationRuns({ scenario, modelVersion });
...
export async function createMaValuationRun({ eventId, asOfDate, scenario = "Base", modelVersion = DEFAULT_MODEL_VERSION, assumptions = {} } = {}) {
  const rawSnapshot = getMaSnapshot({ eventId, asOfDate });
  if (!rawSnapshot.reportingEvent) {
    throw new Error("No MA reporting event matched the supplied eventId/asOfDate.");
  }
  ...
  const valuationResult = await runMaBackendValuation({
    snapshot,
    scenario,
    modelVersion,
  ...
  execute(
    ...
    [
      ...
      JSON.stringify([
        ...
        ...(dailyPrice
          ? [{
              id: "ma-daily-price-anchor",
              title: "MA daily price anchor",
          ...
          : [{
              id: "ma-missing-daily-price-anchor",
              title: "MA daily price missing",
  ...
  return {
    id,
    persisted: true,
    valuationRun: getMaValuationRuns({ eventId: snapshot.reportingEvent.id, scenario, modelVersion })[0],
    valuationResult,
  };
}

export async function backfillMaValuationRuns({
  scenarios = ["Bear", "Base", "Bull"],
  modelVersion = DEFAULT_MODEL_VERSION,
  replace = true,
} = {}) {
  if (replace) {
    execute("DELETE FROM valuation_runs WHERE ticker = ? AND modelVersion = ?", [TICKER, modelVersion], MA_BACKEND_DB_PATH);
  }
  const events = getMaReportingEvents().slice().sort((left, right) => left.eventDate.localeCompare(right.eventDate));
  const created = [];
  const failed = [];
  for (const event of events) {
    for (const scenario of scenarios) {
      try {
        const result = await createMaValuationRun({ eventId: event.id, scenario, modelVersion });
        created.push({
          eventId: event.id,
