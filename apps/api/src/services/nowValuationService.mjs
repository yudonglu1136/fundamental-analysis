...
import { getNowReportingEvents, getNowSnapshot } from "./nowSnapshotService.mjs";
import { NOW_BACKEND_DB_PATH } from "../../../../modules/now/db/schema.mjs";
import { runNowBackendValuation } from "../../../../modules/now/valuation/adapter.mjs";
...
function applyDailyPriceToSnapshot(snapshot) {
  ...
  return {
    ...
    snapshot: {
      ...
      marketSnapshot: {
        ...marketSnapshot,
        id: marketSnapshot.id ?? `now-daily-price-${snapshot.asOfDate}`,
        ticker: TICKER,
...
export function getNowValuationRuns({ asOfDate, eventId, scenario, modelVersion } = {}) {
  ...
  return query(`SELECT * FROM valuation_runs WHERE ${clauses.join(" AND ")} ORDER BY createdAt DESC`, params, NOW_BACKEND_DB_PATH).map((row) => ({
    ...row,
...
export function getNowHistoricalValuations({ scenario = "Base", modelVersion = DEFAULT_MODEL_VERSION } = {}) {
  const events = getNowReportingEvents();
  const runs = getNowValuationRuns({ scenario, modelVersion });
...
export async function createNowValuationRun({ eventId, asOfDate, scenario = "Base", modelVersion = DEFAULT_MODEL_VERSION, assumptions = {} } = {}) {
  const rawSnapshot = getNowSnapshot({ eventId, asOfDate });
  if (!rawSnapshot.reportingEvent) {
    throw new Error("No NOW reporting event matched the supplied eventId/asOfDate.");
  }
  ...
  const valuationResult = await runNowBackendValuation({
    snapshot,
    scenario,
    modelVersion,
  ...
  execute(
    ...
    [
      ...
      JSON.stringify([
        ...
        ...(dailyPrice
          ? [{
              id: "now-daily-price-anchor",
              title: "NOW daily price anchor",
          ...
          : [{
              id: "now-missing-daily-price-anchor",
              title: "NOW daily price missing",
  ...
  return {
    id,
    persisted: true,
    valuationRun: getNowValuationRuns({ eventId: snapshot.reportingEvent.id, scenario, modelVersion })[0],
    valuationResult,
  };
}

export async function backfillNowValuationRuns({
  scenarios = ["Bear", "Base", "Bull"],
  modelVersion = DEFAULT_MODEL_VERSION,
  replace = true,
} = {}) {
  if (replace) {
    execute("DELETE FROM valuation_runs WHERE ticker = ? AND modelVersion = ?", [TICKER, modelVersion], NOW_BACKEND_DB_PATH);
  }
  const events = getNowReportingEvents().slice().sort((left, right) => left.eventDate.localeCompare(right.eventDate));
  const created = [];
  const failed = [];
  for (const event of events) {
    for (const scenario of scenarios) {
      try {
        const result = await createNowValuationRun({ eventId: event.id, scenario, modelVersion });
        created.push({
          eventId: event.id,
