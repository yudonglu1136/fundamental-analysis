...
import { getAaplReportingEvents, getAaplSnapshot } from "./aaplSnapshotService.mjs";
import { AAPL_BACKEND_DB_PATH } from "../../../../modules/aapl/db/schema.mjs";
import { runAaplBackendValuation } from "../../../../modules/aapl/valuation/adapter.mjs";
import { AAPL_BACKEND_MODEL_VERSION } from "../../../../modules/aapl/valuation/modelVersion.mjs";

const TICKER = "AAPL";
const DEFAULT_MODEL_VERSION = AAPL_BACKEND_MODEL_VERSION.version;
...
function applyDailyPriceToSnapshot(snapshot) {
  ...
  return {
    ...
    snapshot: {
      ...
      marketSnapshot: {
        ...marketSnapshot,
        id: marketSnapshot.id ?? `aapl-daily-price-${snapshot.asOfDate}`,
        ticker: TICKER,
...
export function getAaplValuationRuns({ asOfDate, eventId, scenario, modelVersion } = {}) {
  ...
  return query(`SELECT * FROM valuation_runs WHERE ${clauses.join(" AND ")} ORDER BY createdAt DESC`, params, AAPL_BACKEND_DB_PATH).map((row) => ({
    ...row,
...
export function getAaplHistoricalValuations({ scenario = "Base", modelVersion = DEFAULT_MODEL_VERSION } = {}) {
  const events = getAaplReportingEvents();
  const runs = getAaplValuationRuns({ scenario, modelVersion });
...
export async function createAaplValuationRun({ eventId, asOfDate, scenario = "Base", modelVersion = DEFAULT_MODEL_VERSION, assumptions = {} } = {}) {
  const rawSnapshot = getAaplSnapshot({ eventId, asOfDate });
  if (!rawSnapshot.reportingEvent) {
    throw new Error("No AAPL reporting event matched the supplied eventId/asOfDate.");
  }
  ...
  const valuationResult = await runAaplBackendValuation({
    snapshot,
    scenario,
    modelVersion,
  ...
  const targetPrice3Y = selectedScenario?.targetPrice3Y ?? valuationResult.targetPrice3Y ?? (fairValue != null ? fairValue * 1.08 : null);
  const cumulativeDividends = selectedScenario?.cumulativeDividends ?? (currentPrice ? currentPrice * 0.005 * 3 : 0);
  ...
  execute(
    ...
    [
      ...
      JSON.stringify([
        ...
        ...(dailyPrice
          ? [{
              id: "aapl-daily-price-anchor",
              title: "AAPL daily price anchor",
              detail: `As-of price uses ${dailyPrice.source} adjusted close from ${dailyPrice.priceDate}.`,
              severity: "low",
            }]
          : [{
              id: "aapl-missing-daily-price-anchor",
              title: "AAPL daily price missing",
  ...
  return {
    id,
    persisted: true,
    valuationRun: getAaplValuationRuns({ eventId: snapshot.reportingEvent.id, scenario, modelVersion })[0],
    valuationResult,
  };
}

export async function backfillAaplValuationRuns({
  scenarios = ["Bear", "Base", "Bull"],
  modelVersion = DEFAULT_MODEL_VERSION,
  replace = true,
} = {}) {
  if (replace) {
    execute("DELETE FROM valuation_runs WHERE ticker = ? AND modelVersion = ?", [TICKER, modelVersion], AAPL_BACKEND_DB_PATH);
  }
  const events = getAaplReportingEvents().slice().sort((left, right) => left.eventDate.localeCompare(right.eventDate));
  const created = [];
  const failed = [];
  for (const event of events) {
    for (const scenario of scenarios) {
      try {
        const result = await createAaplValuationRun({ eventId: event.id, scenario, modelVersion });
        created.push({
          eventId: event.id,
