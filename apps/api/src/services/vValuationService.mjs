...
import { getVReportingEvents, getVSnapshot } from "./vSnapshotService.mjs";
...
function applyDailyPriceToSnapshot(snapshot) {
  ...
  return {
    ...
    snapshot: {
      ...
      marketSnapshot: {
        ...marketSnapshot,
        id: marketSnapshot.id ?? `v-daily-price-${snapshot.asOfDate}`,
        ticker: TICKER,
...
export function getVValuationRuns({ asOfDate, eventId, scenario, modelVersion } = {}) {
  ...
  return query(`SELECT * FROM valuation_runs WHERE ${clauses.join(" AND ")} ORDER BY createdAt DESC`, params, V_BACKEND_DB_PATH).map((row) => ({
    ...row,
...
export function getVHistoricalValuations({ scenario = "Base", modelVersion = DEFAULT_MODEL_VERSION } = {}) {
  const events = getVReportingEvents();
  const runs = getVValuationRuns({ scenario, modelVersion });
...
export async function createVValuationRun({ eventId, asOfDate, scenario = "Base", modelVersion = DEFAULT_MODEL_VERSION, assumptions = {} } = {}) {
  const rawSnapshot = getVSnapshot({ eventId, asOfDate });
  if (!rawSnapshot.reportingEvent) {
    throw new Error("No V reporting event matched the supplied eventId/asOfDate.");
  }
  ...
  const valuationResult = await runVBackendValuation({
    snapshot,
    scenario,
    modelVersion,
  ...
  execute(
    ...
    [
      ...
      JSON.stringify([
        ...
        ...(dailyPrice
          ? [{
              id: "v-daily-price-anchor",
              title: "V daily price anchor",
          ...
          : [{
              id: "v-missing-daily-price-anchor",
              title: "V daily price missing",
  ...
  return {
    id,
    persisted: true,
    valuationRun: getVValuationRuns({ eventId: snapshot.reportingEvent.id, scenario, modelVersion })[0],
    valuationResult,
  };
}

export async function backfillVValuationRuns({
  scenarios = ["Bear", "Base", "Bull"],
  modelVersion = DEFAULT_MODEL_VERSION,
  replace = true,
} = {}) {
  if (replace) {
    execute("DELETE FROM valuation_runs WHERE ticker = ? AND modelVersion = ?", [TICKER, modelVersion], V_BACKEND_DB_PATH);
  }
  const events = getVReportingEvents().slice().sort((left, right) => left.eventDate.localeCompare(right.eventDate));
  const created = [];
  const failed = [];
  for (const event of events) {
    for (const scenario of scenarios) {
      try {
        const result = await createVValuationRun({ eventId: event.id, scenario, modelVersion });
        created.push({
          eventId: event.id,
