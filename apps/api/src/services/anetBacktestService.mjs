...
export function getAnetBacktests() {
  return query(
...
export function runAnetBacktest({
  startDate = "2018-01-02",
  endDate = "2026-05-12",
  modelVersion = DEFAULT_MODEL_VERSION,
  benchmarkTicker = "SPY",
} = {}) {
  const anetBars = getPriceBars(TICKER, startDate, endDate);
  const benchmarkBars = getPriceBars(benchmarkTicker, startDate, endDate);
  const warnings = [];
  if (anetBars.length < 2) warnings.push("ANET daily adjusted price history is unavailable or too short for the selected window.");
  ...
  let anetBuyHoldValue = 1;
  let benchmarkValue = 1;
  let previousAnet = anetBars[0];
  let previousBenchmark = benchmarkByDate.get(anetBars[0].priceDate) ?? benchmarkBars[0];
  const curve = [];

  for (let index = 0; index < anetBars.length; index += 1) {
    const anet = anetBars[index];
    const benchmark = benchmarkByDate.get(anet.priceDate);
    if (!benchmark) continue;
    if (curve.length) {
      const anetReturn = anet.adjustedClose / previousAnet.adjustedClose - 1;
      const benchmarkReturn = benchmark.adjustedClose / previousBenchmark.adjustedClose - 1;
      anetBuyHoldValue *= 1 + anetReturn;
      benchmarkValue *= 1 + benchmarkReturn;
    }
    curve.push({
      date: anet.priceDate,
      anetBuyHold: anetBuyHoldValue,
      spy: benchmarkValue,
      benchmark: benchmarkValue,
      price: anet.adjustedClose,
    });
    previousAnet = anet;
  ...
  const result = {
    ...
    priceBars: {
      ...
      [benchmarkTicker]: benchmarkBars.length,
      sources: {
        [TICKER]: anetBars[0]?.source ?? null,
        [benchmarkTicker]: benchmarkBars[0]?.source ?? null,
      },
    },
    metrics: {
      anetBuyHold: metrics(curve.map((row) => ({ date: row.date, value: row.anetBuyHold }))),
