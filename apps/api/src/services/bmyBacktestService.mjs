...
export function getBmyBacktests() {
  return query(
...
export function runBmyBacktest({
  startDate = "2018-01-02",
  endDate = "2026-05-12",
  modelVersion = BMY_BACKEND_MODEL_VERSION.version,
  benchmarkTicker = "SPY",
} = {}) {
  const bmyBars = getPriceBars(TICKER, startDate, endDate);
  const benchmarkBars = getPriceBars(benchmarkTicker, startDate, endDate);
  const warnings = [];
  if (bmyBars.length < 2) warnings.push("BMY daily price history is unavailable or too short for the selected window.");
  ...
  let bmyBuyHoldValue = 1;
  let benchmarkValue = 1;
  let previousBmy = bmyBars[0];
  let previousBenchmark = benchmarkByDate.get(bmyBars[0].priceDate) ?? benchmarkBars[0];
  const curve = [];

  for (const bmy of bmyBars) {
    const benchmark = benchmarkByDate.get(bmy.priceDate);
    if (!benchmark) continue;
    if (curve.length) {
      bmyBuyHoldValue *= 1 + (bmy.adjustedClose / previousBmy.adjustedClose - 1);
  ...
  const result = {
    ...
    priceBars: {
      ...
      [benchmarkTicker]: benchmarkBars.length,
      sources: {
        [TICKER]: bmyBars[0]?.source ?? null,
        [benchmarkTicker]: benchmarkBars[0]?.source ?? null,
      },
    },
    metrics: {
      bmyBuyHold: metrics(curve.map((row) => ({ date: row.date, value: row.bmyBuyHold }))),
