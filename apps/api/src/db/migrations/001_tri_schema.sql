...
CREATE TABLE IF NOT EXISTS financial_periods (
  ...
  sourceType TEXT,
  revenue REAL,
  organicRevenueGrowth REAL,
  recurringRevenue REAL,
  ...
  fxImpact REAL,
  currentPrice REAL,
  researchOnly INTEGER NOT NULL DEFAULT 0,
  rawJson TEXT
);
...
CREATE INDEX IF NOT EXISTS idx_tri_reporting_events_ticker_date ON reporting_events(ticker, eventDate);
CREATE INDEX IF NOT EXISTS idx_tri_financial_periods_event ON financial_periods(ticker, eventId, asOfDate);
CREATE INDEX IF NOT EXISTS idx_tri_segment_financials_event ON segment_financials(ticker, eventId, asOfDate);
CREATE INDEX IF NOT EXISTS idx_tri_market_snapshots_asof ON market_snapshots(ticker, asOfDate);
CREATE INDEX IF NOT EXISTS idx_tri_valuation_runs_lookup ON valuation_runs(ticker, asOfDate, reportingEventId, scenario, modelVersion);
CREATE INDEX IF NOT EXISTS idx_tri_daily_price_bars_ticker_date ON daily_price_bars(ticker, priceDate);
