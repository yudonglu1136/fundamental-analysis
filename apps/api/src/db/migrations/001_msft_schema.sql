...
CREATE TABLE IF NOT EXISTS cloud_ai_kpis (
  ...
  asOfDate TEXT NOT NULL,
  sourceType TEXT,
  microsoftCloudRevenue REAL,
  microsoftCloudGrowth REAL,
  microsoftCloudConstantCurrencyGrowth REAL,
  microsoftCloudGrossMargin REAL,
  azureGrowth REAL,
  azureConstantCurrencyGrowth REAL,
  azureAiContribution REAL,
  m365CommercialCloudGrowth REAL,
  m365CommercialSeatGrowth REAL,
  commercialRpo REAL,
  commercialBookingsGrowth REAL,
  aiArr REAL,
  copilotPaidSeats REAL,
  capexIntensity REAL,
  reportedFcfMargin REAL,
  openAiInvestmentImpact REAL,
  notes TEXT,
  rawJson TEXT
);
...
CREATE INDEX IF NOT EXISTS idx_msft_daily_price_bars_ticker_date
...
CREATE INDEX IF NOT EXISTS idx_msft_reporting_events_ticker_date ON reporting_events(ticker, eventDate);
CREATE INDEX IF NOT EXISTS idx_msft_financial_periods_event ON financial_periods(ticker, eventId, asOfDate);
CREATE INDEX IF NOT EXISTS idx_msft_segment_financials_event ON segment_financials(ticker, eventId, asOfDate);
CREATE INDEX IF NOT EXISTS idx_msft_cloud_ai_kpis_asof ON cloud_ai_kpis(ticker, asOfDate);
CREATE INDEX IF NOT EXISTS idx_msft_market_snapshots_asof ON market_snapshots(ticker, asOfDate);
CREATE INDEX IF NOT EXISTS idx_msft_valuation_runs_lookup ON valuation_runs(ticker, asOfDate, reportingEventId, scenario, modelVersion);
