...
| VAL-047 | P0 | fixed | Application restart could replace the full PIT dashboard with a stale bundled snapshot | Persistent AWS and local runtime databases when the bundled dashboard is older or has lower ticker coverage | Bundled valuation snapshots now install monotonically: every row must be newer, and a dashboard update may never reduce tracked-ticker coverage. Regression tests prove that the old zero-ticker bundle cannot overwrite the 533-ticker PIT release after a restart. |
