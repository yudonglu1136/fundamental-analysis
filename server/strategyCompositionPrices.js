...
// Same exact share-class punctuation aliases as harvest-all-local-prices.py.
// These are provider spellings, never substitutions between share classes.
const chartSymbols=Object.freeze({'BRK.A':'BRK-A','BRK.B':'BRK-B','BF.A':'BF-A','BF.B':'BF-B','HEI.A':'HEI-A','LEN.B':'LEN-B'});
const chartSymbol=symbol=>chartSymbols[symbol]??symbol;
...
export function linkedStrategyComparisonPrices(file,data,end) {
  ...
  try {
    for(const symbol of needed) {
      ...
      if(!series||series.currency!=='USD'||series.provider_symbol!==symbol||!series.source_sha256)continue;
      ...
      const provenance={symbol,currency:'USD',provider:'Yahoo daily chart',field:'close',basis:'split_adjusted_close_not_total_return',sourceHash:series.source_sha256,
...
export function compositionPrices(file,data,end) {
  ...
  try {
    for(const symbol of priceMaps.keys()) {
      ...
      const series=db.prepare('SELECT * FROM series WHERE symbol=?').get(symbol);
      if(!series||series.currency!=='USD'||chartSymbol(symbol)!==series.provider_symbol||!series.source_sha256)continue;
      const meta=JSON.parse(series.metadata_json);
      if(meta.symbol!==chartSymbol(symbol)||meta.currency!=='USD'||!['EQUITY','ETF'].includes(meta.instrumentType))continue;
      ...
      evidence[symbol]={provider:'Yahoo daily chart',providerSymbol:series.provider_symbol,sourceHash:series.source_sha256,pointsHash:signature(rows),first:rows[0].date,last:rows.at(-1).date,rows:rows.length,returnBasis:'single_vintage_total_return_adjusted_close'};
    }
