...
import { buildAmznBackendSeedPayload } from "../ingestion/importLocalData.mjs";
import { AMZN_BACKEND_DB_PATH, AMZN_BACKEND_SCHEMA_PATH, AMZN_BACKEND_TABLES } from "./schema.mjs";
...
export async function seedAmznBackendDb() {
  mkdirSync(path.dirname(AMZN_BACKEND_DB_PATH), { recursive: true });
  const payload = await buildAmznBackendSeedPayload();
  const result = spawnSync("python3", ["-c", seedPython], {
    input: JSON.stringify({
      dbPath: AMZN_BACKEND_DB_PATH,
      schema: readFileSync(AMZN_BACKEND_SCHEMA_PATH, "utf8"),
      tablesToClear: AMZN_BACKEND_TABLES,
      tables: {
        ...
        business_unit_financials: payload.businessUnitFinancials,
