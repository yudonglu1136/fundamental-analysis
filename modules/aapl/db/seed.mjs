...
import { buildAaplBackendSeedPayload } from "../ingestion/importLocalData.mjs";
import { AAPL_BACKEND_DB_PATH, AAPL_BACKEND_SCHEMA_PATH, AAPL_BACKEND_TABLES } from "./schema.mjs";
...
export async function seedAaplBackendDb() {
  mkdirSync(path.dirname(AAPL_BACKEND_DB_PATH), { recursive: true });
  const payload = await buildAaplBackendSeedPayload();
  const result = spawnSync("python3", ["-c", seedPython], {
    input: JSON.stringify({
      dbPath: AAPL_BACKEND_DB_PATH,
      schema: readFileSync(AAPL_BACKEND_SCHEMA_PATH, "utf8"),
      tablesToClear: AAPL_BACKEND_TABLES,
      tables: {
        ...
        geographic_financials: payload.geographicFinancials,
