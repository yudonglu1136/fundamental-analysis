...
import { buildRtxBackendSeedPayload } from "../ingestion/importLocalData.mjs";
import { RTX_BACKEND_DB_PATH, RTX_BACKEND_SCHEMA_PATH, RTX_BACKEND_TABLES } from "./schema.mjs";
...
export async function seedRtxBackendDb() {
  mkdirSync(path.dirname(RTX_BACKEND_DB_PATH), { recursive: true });
  const payload = await buildRtxBackendSeedPayload();
  const result = spawnSync("python3", ["-c", seedPython], {
    input: JSON.stringify({
      dbPath: RTX_BACKEND_DB_PATH,
      schema: readFileSync(RTX_BACKEND_SCHEMA_PATH, "utf8"),
      tablesToClear: RTX_BACKEND_TABLES,
      tables: {
