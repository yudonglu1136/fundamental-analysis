...
import { buildCegBackendSeedPayload } from "../ingestion/importLocalData.mjs";
import { CEG_BACKEND_DB_PATH, CEG_BACKEND_SCHEMA_PATH, CEG_BACKEND_TABLES } from "./schema.mjs";
...
export async function seedCegBackendDb() {
  mkdirSync(path.dirname(CEG_BACKEND_DB_PATH), { recursive: true });
  const payload = await buildCegBackendSeedPayload();
  const result = spawnSync("python3", ["-c", seedPython], {
    input: JSON.stringify({
      dbPath: CEG_BACKEND_DB_PATH,
      schema: readFileSync(CEG_BACKEND_SCHEMA_PATH, "utf8"),
      tablesToClear: CEG_BACKEND_TABLES,
      tables: {
