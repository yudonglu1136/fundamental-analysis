...
import { buildAnetBackendSeedPayload } from "../ingestion/importLocalData.mjs";
import { ANET_BACKEND_DB_PATH, ANET_BACKEND_SCHEMA_PATH, ANET_BACKEND_TABLES } from "./schema.mjs";
...
export async function seedAnetBackendDb() {
  mkdirSync(path.dirname(ANET_BACKEND_DB_PATH), { recursive: true });
  const payload = await buildAnetBackendSeedPayload();
  const result = spawnSync("python3", ["-c", seedPython], {
    input: JSON.stringify({
      dbPath: ANET_BACKEND_DB_PATH,
      schema: readFileSync(ANET_BACKEND_SCHEMA_PATH, "utf8"),
      tablesToClear: ANET_BACKEND_TABLES,
      tables: {
