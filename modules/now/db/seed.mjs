...
import { buildNowBackendSeedPayload } from "../ingestion/importLocalData.mjs";
import { NOW_BACKEND_DB_PATH, NOW_BACKEND_SCHEMA_PATH, NOW_BACKEND_TABLES } from "./schema.mjs";
...
export async function seedNowBackendDb() {
  mkdirSync(path.dirname(NOW_BACKEND_DB_PATH), { recursive: true });
  const payload = await buildNowBackendSeedPayload();
  const result = spawnSync("python3", ["-c", seedPython], {
    input: JSON.stringify({
      dbPath: NOW_BACKEND_DB_PATH,
      schema: readFileSync(NOW_BACKEND_SCHEMA_PATH, "utf8"),
      tablesToClear: NOW_BACKEND_TABLES,
      tables: {
