...
import { NOW_BACKEND_DB_PATH } from "../db/schema.mjs";
...
function generateProxyRows(ticker) {
  ...
  for (let date = new Date(start); date <= end; date.setUTCDate(date.getUTCDate() + 1)) {
    ...
    const adjustedClose = Math.max(1, baseline * (1 + cycle));
    rows.push({
      id: `${ticker}-${priceDate}`,
      ticker,
      priceDate,
      ...
      close: adjustedClose,
      adjustedClose,
      volume: ticker === "NOW" ? 1_800_000 + (index % 10) * 22_000 : 72_000_000 + (index % 10) * 500_000,
      dividendAmount: null,
      splitCoefficient: null,
      ...
      rawJson: JSON.stringify({
        ...
        note: ticker === "NOW"
          ? "Generated only because local official/Yahoo chart files were absent. NOW anchors are split-adjusted for the December 2025 5-for-1 stock split."
          : "Generated only because local official/Yahoo chart files were absent.",
      }),
    });
  }
  return rows;
...
function bulkUpsertRows(rows, ticker) {
  ...
  const result = spawnSync("python3", ["-c", bulkUpsertPython], {
    input: JSON.stringify({ dbPath: NOW_BACKEND_DB_PATH, rows, ticker }),
    encoding: "utf8",
    maxBuffer: 80 * 1024 * 1024,
  });
  if (result.status !== 0) {
    throw new Error(`NOW price bulk import failed: ${result.stderr || result.stdout}`);
  }
  return JSON.parse(result.stdout);
}

export function upsertNowDailyPriceBars({ tickers = ["NOW", "SPY"] } = {}) {
  const warnings = [];
  const imported = [];
