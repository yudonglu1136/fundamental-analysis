import { NOW_BACKEND_MODEL_VERSION } from "./modelVersion.mjs";
...
function valueForScenario(name, baseline, assumptions) {
  ...
  const margin = clamp(
    drivers.operatingMargin + (subscriptionGrowth - 0.18) * 0.08 + proPlusAdoptionRate * 0.04 - drivers.platformCompetitionHaircut * 0.10 - drivers.sbcDilutionHaircut * 0.06,
    0.08,
    0.25,
  );
  const fcfMargin = clamp(drivers.normalizedFcfMargin + (margin - 0.16) * 0.45 - drivers.sbcDilutionHaircut * 0.20, 0.24, 0.42);
...
export async function runNowBackendValuation({
  snapshot,
  scenario = "Base",
  ...
  if (!baseline) {
    throw new Error("NOW backend valuation requires at least one event-visible financial_periods row.");
  }
  ...
  const warnings = [
    warning(
      "now-proxy-seed-data",
      "NOW official parser pending",
      "Financial and operating metric rows are tagged official_seed or market_data_proxy until the ServiceNow SEC/companyfacts parser is promoted.",
      "medium",
    ),
    ...(baseline.windowRows.length < 4
      ? [warning("now-partial-ttm", "Partial historical window", `${baseline.selected.periodId} valuation annualizes ${baseline.windowRows.length} quarter(s) visible as of the event date.`, "medium")]
      : []),
    ...(baseline.agenticAiArr != null && baseline.agenticAiArr > 0 && baseline.selected.asOfDate < "2023-01-01"
      ? [warning("now-agent-ai-leakage-check", "Agentic AI timing check", "Agentic AI metrics before 2023 should be zero or absent; review seed rows if this warning appears.", "high")]
      : []),
  ];

  const methodCards = [
    methodCard("dcf", "DCF / FCFF", methodValues.dcfValue, "Five-year FCFF fade with event-visible subscription growth, cRPO growth, operating leverage, terminal growth, and discount rate.", "normalized TTM FCF"),
    ...
    methodCard("ev-revenue", "EV/Revenue", methodValues.evRevenueValue, "Forward revenue capitalized at a SaaS growth-and-margin multiple and bridged to equity value.", "next-year revenue"),
    methodCard("peer-premium", "Enterprise SaaS Peer Premium", methodValues.peerValue, "Enterprise SaaS / workflow peer reference with explicit premium or discount for AI adoption and platform risk.", "peer EPS multiple"),
  ];

  const sensitivityTables = [
    {
      title: "Agent AI Adoption vs Platform Risk",
      table: [
        ["Agent / Risk", "Low risk", "Base risk", "High risk"],
  ...
  return {
    ...
    expectedReturnBridge: [
      { key: "subscription-growth", label: "Subscription growth", value: methodFacts.subscriptionGrowth, format: "percent", description: "Event-visible subscription growth blended with scenario demand." },
      { key: "crpo-growth", label: "cRPO growth", value: methodFacts.currentRpoGrowth, format: "percent", description: "Forward demand check from current remaining performance obligations." },
      ...
      { key: "ai-risk", label: "AI execution haircut", value: selected.drivers.aiExecutionHaircut, format: "percent", description: "Explicit haircut for Agentic AI adoption and monetization risk." },
    ],
    backendSnapshot: {
      ticker: "NOW",
