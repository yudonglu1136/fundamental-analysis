...
import { buildLsegBackendSeedPayload } from "../ingestion/importLocalData.mjs";

const dbPath = path.resolve(process.env.LSEG_DB_PATH ?? "data/local/lseg/backend/lseg_research.sqlite");
const schemaPath = path.resolve("apps/api/src/db/migrations/001_lseg_schema.sql");
...
export function seedLsegBackendDb() {
  mkdirSync(path.dirname(dbPath), { recursive: true });
  const payload = buildLsegBackendSeedPayload();
