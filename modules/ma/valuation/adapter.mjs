import { MA_BACKEND_MODEL_VERSION } from "./modelVersion.mjs";
...
export async function runMaBackendValuation({
  snapshot,
  scenario = "Base",
  ...
  if (!baseline) {
    throw new Error("MA backend valuation requires at least one event-visible financial_periods row.");
  }
  ...
  const warnings = [
    warning(
      "ma-proxy-seed-data",
      "MA official parser pending",
      "Financial and operating metric rows are tagged official_seed or market_data_proxy until the Mastercard SEC/companyfacts parser is promoted.",
      "medium",
    ),
    ...(baseline.windowRows.length < 4
      ? [warning("ma-partial-ttm", "Partial historical window", `${baseline.selected.periodId} valuation annualizes ${baseline.windowRows.length} quarter(s) visible as of the event date.`, "medium")]
      : []),
    ...(baseline.crossBorderVolumeGrowth != null && baseline.crossBorderVolumeGrowth < 0
      ? [warning("ma-cross-border-shock", "Cross-border travel shock", "This historical event uses the then-visible cross-border decline rather than later recovery assumptions.", "low")]
      : []),
  ];
  ...
  const methodCards = [
    ...
    methodCard("peer-premium", "Payments Peer Premium", methodValues.peerValue, "Visa/Amex/processor peer reference with explicit premium or discount for MA growth and risk framing.", "peer EPS multiple"),
  ];
