...
import { buildMaBackendSeedPayload } from "../ingestion/importLocalData.mjs";
import { MA_BACKEND_DB_PATH, MA_BACKEND_SCHEMA_PATH, MA_BACKEND_TABLES } from "./schema.mjs";
...
export async function seedMaBackendDb() {
  mkdirSync(path.dirname(MA_BACKEND_DB_PATH), { recursive: true });
  const payload = await buildMaBackendSeedPayload();
  const result = spawnSync("python3", ["-c", seedPython], {
    input: JSON.stringify({
      dbPath: MA_BACKEND_DB_PATH,
      schema: readFileSync(MA_BACKEND_SCHEMA_PATH, "utf8"),
      tablesToClear: MA_BACKEND_TABLES,
      tables: {
