...
import { buildMsftBackendSeedPayload } from "../ingestion/importLocalData.mjs";
import { MSFT_BACKEND_DB_PATH, MSFT_BACKEND_SCHEMA_PATH, MSFT_BACKEND_TABLES } from "./schema.mjs";
...
export async function seedMsftBackendDb() {
  mkdirSync(path.dirname(MSFT_BACKEND_DB_PATH), { recursive: true });
  const payload = await buildMsftBackendSeedPayload();
  const result = spawnSync("python3", ["-c", seedPython], {
    input: JSON.stringify({
      dbPath: MSFT_BACKEND_DB_PATH,
      schema: readFileSync(MSFT_BACKEND_SCHEMA_PATH, "utf8"),
      tablesToClear: MSFT_BACKEND_TABLES,
      tables: {
