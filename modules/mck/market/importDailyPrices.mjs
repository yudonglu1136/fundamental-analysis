...
import { MCK_BACKEND_DB_PATH } from "../db/schema.mjs";

const MARKET_DIR = path.resolve("data/local/mck/market");
  ...
  const rows = series.map((row) => {
    ...
    return {
      ...
      rawJson: JSON.stringify({
        ...row,
        adjustedClosePolicy: "Nasdaq payload did not include adjusted close; close is used as adjustedClose with a backend warning.",
      }),
    };
  });
  return { rows, warning: `${ticker} Nasdaq daily rows use close as adjustedClose because adjusted close is unavailable.` };
}
...
function bulkUpsertRows(rows, ticker) {
  ...
  const result = spawnSync("python3", ["-c", bulkUpsertPython], {
    input: JSON.stringify({ dbPath: MCK_BACKEND_DB_PATH, rows, ticker }),
    encoding: "utf8",
    maxBuffer: 30 * 1024 * 1024,
  });
  if (result.status !== 0) {
    throw new Error(`MCK price bulk import failed: ${result.stderr || result.stdout}`);
  }
  return JSON.parse(result.stdout);
}

export function upsertMckDailyPriceBars({ tickers = ["MCK", "SPY"] } = {}) {
  const warnings = [];
  const imported = [];
