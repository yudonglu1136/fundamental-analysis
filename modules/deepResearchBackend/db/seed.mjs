...
import { buildDeepResearchBackendSeedPayload } from "../ingestion/importLocalData.mjs";
import { DEEP_RESEARCH_BACKEND_SCHEMA_PATH, DEEP_RESEARCH_BACKEND_TABLES } from "./schema.mjs";
...
export async function seedDeepResearchBackendDb(slugOrTicker) {
  const payload = buildDeepResearchBackendSeedPayload(slugOrTicker);
  mkdirSync(path.dirname(payload.profile.dbPath), { recursive: true });
  const result = spawnSync("python3", ["-c", seedPython], {
    input: JSON.stringify({
      dbPath: payload.profile.dbPath,
      schema: readFileSync(DEEP_RESEARCH_BACKEND_SCHEMA_PATH, "utf8"),
      tablesToClear: DEEP_RESEARCH_BACKEND_TABLES,
      tables: {
        ...
        daily_price_bars: payload.dailyPriceBars,
      },
    }),
    encoding: "utf8",
