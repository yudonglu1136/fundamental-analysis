...
import { META_BACKEND_DB_PATH } from "../db/schema.mjs";

const MARKET_DIR = path.resolve("data/local/meta/market");
const FALLBACK_MARKET_DIRS = [
  ...
  path.resolve("data/local/tri/market"),
];
...
function parseNasdaqHistorical(filePath, ticker) {
  ...
  try {
    ...
    const rows = series
      .map((row) => {
        ...
        return {
          ...
          source: "Nasdaq historical quote API local cache",
    ...
    return {
      rows,
      warning: `${ticker} Nasdaq local cache does not include adjusted close; close is used as adjustedClose and returns are price-return only.`,
    };
  } catch (error) {
    return { rows: [], warning: `${ticker} Nasdaq parse failed: ${error instanceof Error ? error.message : String(error)}` };
  }
}
...
async function fetchStooq(ticker) {
  const symbol = ticker === "SPY" ? "spy.us" : "meta.us";
  const url = `https://stooq.com/q/d/l/?s=${symbol}&i=d&d1=20180102&d2=20260512`;
  try {
    const response = await fetch(url, { headers: { "User-Agent": "fundamental-analysis-meta-backend/1.0" } });
...
function sourcesForTicker(ticker) {
  ...
  for (const directory of FALLBACK_MARKET_DIRS) {
    ...
    localSources.push(() => parseNasdaqHistorical(path.join(directory, `nasdaq_${lower}_historical.json`), ticker));
  }
  return localSources;
}
...
function generateProxyRows(ticker) {
  const anchors = ticker === "SPY" ? SPY_ANCHORS : META_ANCHORS;
  const rows = [];
  ...
  while (cursor <= end) {
    if (isWeekday(cursor)) {
      ...
      const wave = Math.sin(rows.length / 13) * 0.012 + Math.cos(rows.length / 37) * 0.008;
      ...
      rows.push({
        ...
        high: Number((close * 1.008).toFixed(4)),
...
function bulkUpsertRows(rows, ticker) {
  ...
  const result = spawnSync("python3", ["-c", bulkUpsertPython], {
    input: JSON.stringify({ dbPath: META_BACKEND_DB_PATH, rows, ticker }),
    encoding: "utf8",
    maxBuffer: 80 * 1024 * 1024,
  });
  if (result.status !== 0) {
    throw new Error(`META price bulk import failed: ${result.stderr || result.stdout}`);
  }
...
export async function upsertMetaDailyPriceBars({ tickers = ["META", "SPY"] } = {}) {
  const warnings = [];
  const imported = [];
