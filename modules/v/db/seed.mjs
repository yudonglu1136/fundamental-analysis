...
import { buildVBackendSeedPayload } from "../ingestion/importLocalData.mjs";
import { V_BACKEND_DB_PATH, V_BACKEND_SCHEMA_PATH, V_BACKEND_TABLES } from "./schema.mjs";
...
export async function seedVBackendDb() {
  mkdirSync(path.dirname(V_BACKEND_DB_PATH), { recursive: true });
  const payload = await buildVBackendSeedPayload();
  const result = spawnSync("python3", ["-c", seedPython], {
    input: JSON.stringify({
      dbPath: V_BACKEND_DB_PATH,
      schema: readFileSync(V_BACKEND_SCHEMA_PATH, "utf8"),
      tablesToClear: V_BACKEND_TABLES,
      tables: {
