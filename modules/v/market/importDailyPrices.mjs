...
import { V_BACKEND_DB_PATH } from "../db/schema.mjs";

const MARKET_DIR = path.resolve("data/local/v/market");
...
function generateProxyRows(ticker) {
  ...
  for (let date = new Date(start); date <= end; date.setUTCDate(date.getUTCDate() + 1)) {
    ...
    rows.push({
      ...
      close: adjustedClose,
      adjustedClose,
      volume: ticker === "V" ? 3_200_000 + (index % 10) * 25_000 : 72_000_000 + (index % 10) * 500_000,
      dividendAmount: null,
      splitCoefficient: null,
...
function bulkUpsertRows(rows, ticker) {
  ...
  const result = spawnSync("python3", ["-c", bulkUpsertPython], {
    input: JSON.stringify({ dbPath: V_BACKEND_DB_PATH, rows, ticker }),
    encoding: "utf8",
    maxBuffer: 80 * 1024 * 1024,
  });
  if (result.status !== 0) {
    throw new Error(`V price bulk import failed: ${result.stderr || result.stdout}`);
  }
  return JSON.parse(result.stdout);
}

export function upsertVDailyPriceBars({ tickers = ["V", "SPY"] } = {}) {
  const warnings = [];
  const imported = [];
