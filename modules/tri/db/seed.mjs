...
import { buildTriBackendSeedPayload } from "../ingestion/importLocalData.mjs";
import { TRI_BACKEND_DB_PATH, TRI_BACKEND_SCHEMA_PATH, TRI_BACKEND_TABLES } from "./schema.mjs";
...
export async function seedTriBackendDb() {
  mkdirSync(path.dirname(TRI_BACKEND_DB_PATH), { recursive: true });
  const payload = await buildTriBackendSeedPayload();
  const result = spawnSync("python3", ["-c", seedPython], {
    input: JSON.stringify({
      dbPath: TRI_BACKEND_DB_PATH,
      schema: readFileSync(TRI_BACKEND_SCHEMA_PATH, "utf8"),
      tablesToClear: TRI_BACKEND_TABLES,
      tables: {
