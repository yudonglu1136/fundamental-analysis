...
import { PLTR_BACKEND_DB_PATH } from "../db/schema.mjs";

const TICKER = "PLTR";
const OUT_DIR = path.resolve("data/local/pltr/market");
...
async function fetchYahooChart() {
  ...
  const response = await fetch(url, {
    headers: { "User-Agent": "fundamental-analysis-pltr-backend-price-importer" },
  });
  if (!response.ok) throw new Error(`Yahoo Finance returned ${response.status}`);
  const json = await response.json();
  const result = json.chart?.result?.[0];
  if (!result?.timestamp?.length) {
    throw new Error(`Yahoo Finance response did not include price timestamps: ${JSON.stringify(json.chart?.error ?? json).slice(0, 500)}`);
  }
  return { url, result, json };
}

export async function importPltrDailyPrices() {
  mkdirSync(OUT_DIR, { recursive: true });
  const { url, result, json } = await fetchYahooChart();
  writeFileSync(path.join(OUT_DIR, "yahoo_pltr_chart.json"), JSON.stringify(json, null, 2));
  ...
  const resultPayload = spawnSync("python3", ["-c", importPython], {
    input: JSON.stringify({ dbPath: PLTR_BACKEND_DB_PATH, rows }),
    encoding: "utf8",
    maxBuffer: 40 * 1024 * 1024,
  });
  if (resultPayload.status !== 0) {
    throw new Error(resultPayload.stderr || resultPayload.stdout);
  }
