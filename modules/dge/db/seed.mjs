...
import { buildDgeBackendSeedPayload } from "../ingestion/importLocalData.mjs";
import { DGE_BACKEND_DB_PATH, DGE_BACKEND_SCHEMA_PATH, DGE_BACKEND_TABLES } from "./schema.mjs";
...
try:
    ...
    for table, rows in payload["tables"].items():
        for row in rows:
            insert(table, row)
    conn.commit()
    count_tables = list(payload["tablesToClear"]) + ["daily_price_bars"]
...
export async function seedDgeBackendDb() {
  mkdirSync(path.dirname(DGE_BACKEND_DB_PATH), { recursive: true });
  const payload = await buildDgeBackendSeedPayload();
  const result = spawnSync("python3", ["-c", seedPython], {
    input: JSON.stringify({
      dbPath: DGE_BACKEND_DB_PATH,
      schema: readFileSync(DGE_BACKEND_SCHEMA_PATH, "utf8"),
      tablesToClear: DGE_BACKEND_TABLES,
      tables: {
