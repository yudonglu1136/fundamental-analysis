...
import { buildGooglBackendSeedPayload } from "../ingestion/importLocalData.mjs";
import { GOOGL_BACKEND_DB_PATH, GOOGL_BACKEND_SCHEMA_PATH, GOOGL_BACKEND_TABLES } from "./schema.mjs";
...
export async function seedGooglBackendDb() {
  mkdirSync(path.dirname(GOOGL_BACKEND_DB_PATH), { recursive: true });
  const payload = await buildGooglBackendSeedPayload();
  const result = spawnSync("python3", ["-c", seedPython], {
    input: JSON.stringify({
      dbPath: GOOGL_BACKEND_DB_PATH,
      schema: readFileSync(GOOGL_BACKEND_SCHEMA_PATH, "utf8"),
      tablesToClear: GOOGL_BACKEND_TABLES,
      tables: {
        ...
        cloud_ai_kpis: payload.cloudAiKpis,
