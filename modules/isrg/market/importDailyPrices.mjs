...
export const ISRG_BACKEND_DB_PATH = path.resolve(process.env.ISRG_DB_PATH ?? "data/local/isrg/backend/isrg_research.sqlite");
const MARKET_DIR = path.resolve("data/local/isrg/market");
...
function parseNasdaqHistorical(filePath, ticker) {
  ...
  const rows = series
    .map((row) => {
      ...
      return {
        ...
        sourceType: "market_data_unadjusted_close",
        fetchedAt: new Date().toISOString(),
        rawJson: JSON.stringify({
          ...row,
          adjustedCloseSource: "close_mirrored_because_nasdaq_payload_has_no_adjusted_close",
        }),
      };
    })
    .filter(Boolean);
  return {
    rows,
    warning: `${ticker} Nasdaq historical quote payload does not include adjusted close. adjustedClose mirrors close, so returns are price-only and not dividend-adjusted.`,
  };
}
...
function bulkUpsertRows(rows, ticker) {
  ...
  const result = spawnSync("python3", ["-c", bulkUpsertPython], {
    input: JSON.stringify({ dbPath: ISRG_BACKEND_DB_PATH, rows, ticker }),
    encoding: "utf8",
    maxBuffer: 40 * 1024 * 1024,
  });
  if (result.status !== 0) {
    throw new Error(`ISRG price bulk import failed: ${result.stderr || result.stdout}`);
  }
  return JSON.parse(result.stdout);
}

export function upsertIsrgDailyPriceBars({ tickers = ["ISRG", "SPY"] } = {}) {
  const warnings = [];
  const imported = [];
  for (const ticker of tickers) {
    ...
    rows = [...rows].sort((left, right) => left.priceDate.localeCompare(right.priceDate));
