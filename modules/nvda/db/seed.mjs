...
import { buildNvdaBackendSeedPayload } from "../ingestion/importLocalData.mjs";
import { NVDA_BACKEND_DB_PATH, NVDA_BACKEND_SCHEMA_PATH, NVDA_BACKEND_TABLES } from "./schema.mjs";
...
try:
    ...
    for table, rows in payload["tables"].items():
        for row in rows:
            insert(table, row)
    conn.commit()
    counts = {table: conn.execute(f"SELECT COUNT(*) FROM {table}").fetchone()[0] for table in payload["tablesToClear"] + ["daily_price_bars"]}
    print(json.dumps({"dbPath": payload["dbPath"], "counts": counts}, indent=2))
finally:
    conn.close()
`;

export async function seedNvdaBackendDb() {
  mkdirSync(path.dirname(NVDA_BACKEND_DB_PATH), { recursive: true });
  const payload = await buildNvdaBackendSeedPayload();
  const result = spawnSync("python3", ["-c", seedPython], {
    input: JSON.stringify({
      dbPath: NVDA_BACKEND_DB_PATH,
      schema: readFileSync(NVDA_BACKEND_SCHEMA_PATH, "utf8"),
      tablesToClear: NVDA_BACKEND_TABLES,
      tables: {
        ...
        customer_end_market_snapshots: payload.customerEndMarketSnapshots,
        supply_chain_snapshots: payload.supplyChainSnapshots,
        operating_metric_snapshots: payload.operatingMetricSnapshots,
